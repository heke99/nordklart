# Hosted Deployment Nordklart with Docker
...
```bash
mkdir Nordklart && cd Nordklart

# Compose file + env template
curl -fsSLO https://raw.githubusercontent.com/nordklart/nordklart/main/docker-compose.yml
curl -fsSLO https://raw.githubusercontent.com/nordklart/nordklart/main/.env.docker.example

# Cron sidecar (Dockerfile + schedule)
mkdir -p docker
curl -fsSL -o docker/cron.Dockerfile \
  https://raw.githubusercontent.com/nordklart/nordklart/main/docker/cron.Dockerfile
curl -fsSL -o docker/crontab.hosted \
  https://raw.githubusercontent.com/nordklart/nordklart/main/docker/crontab.hosted
```
...
| `NEXT_PUBLIC_APP_URL` | The URL where you'll access Nordklart (e.g. `https://nordklart.example.com`) |
...
`nordklart.example.com → <your-public-ip>` (A record). Ports 80 and 443 must be reachable from the internet (Let's Encrypt's HTTP-01 challenge uses port 80).

### 2. Set `DOMAIN` in `.env`

```env
DOMAIN=nordklart.example.com
NEXT_PUBLIC_APP_URL=https://nordklart.example.com
```

### 3. Download the overlay + Caddyfile

```bash
curl -fsSLO https://raw.githubusercontent.com/nordklart/nordklart/main/docker-compose.caddy.yml
mkdir -p docker
curl -fsSL -o docker/Caddyfile \
  https://raw.githubusercontent.com/nordklart/nordklart/main/docker/Caddyfile
```
...
Caddy obtains a cert on first boot (takes ~10 s). Visit `https://nordklart.example.com`.
...
The hosted image ships with all extensions enabled (except Enable Banking, which requires private PSD2 credentials). Each extension activates when you provide its env vars — without them, the app works normally and the feature is simply unavailable.
...
Browse available tags at https://github.com/nordklart/nordklart/pkgs/container/nordklart. For maximum integrity, pin by digest:
...
```bash
# Clone the repo
git clone https://github.com/nordklart/nordklart.git
cd Nordklart
cp .env.docker.example .env
# Fill in .env
...
```nginx
server {
    server_name nordklart.example.com;
