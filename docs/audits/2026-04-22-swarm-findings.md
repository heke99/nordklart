# Audit findings (non-critical) — 2026-04-22

> **Status: frozen snapshot. Do not read as a to-do list.**
>
> This is the output of one `/swarm` run on 2026-04-22, kept for its reasoning
> and for traceability of what was known when. It was never updated afterwards,
> and a number of its findings have since been fixed, superseded by a redesign,
> or were wrong at the time — the MFA, storno-atomicity, entitlement-fallback,
> personnummer and signature-evidence areas in particular were rebuilt in
> August 2026 and no longer resemble what is described below.
>
> Treat every entry as "was true on 2026-04-22 unless verified otherwise".
> Before acting on one, check the current code and the later audits in this
> directory. It lived in the repository root until 2026-08-21, where its size
> and filename made it look like live guidance; moving it here is the fix for
> that, not a re-endorsement of its contents.

Generated by `/swarm` audit on 2026-04-22 (run `20260422-125911`).

**Total non-critical findings**: 324 (~113 high, ~141 medium, ~73 low)

The 35 critical findings from this audit were filed as GitHub issues on [`nordklart/nordklart`](https://github.com/nordklart/nordklart/issues). The full report including criticals is at `.swarm/20260422-125911/findings.md` (gitignored).
...
- 🔁 Possibly tracked by [#243 Projektbaserad](https://github.com/nordklart/nordklart/issues/243)
...
- 🔁 Possibly tracked by [#193 INKS2 balanserar inte](https://github.com/nordklart/nordklart/issues/193)
...
- 🔁 Possibly tracked by [#107 Skatteverket API](https://github.com/nordklart/nordklart/issues/107)
...
*Most findings below possibly tracked by [#243 Projektbaserad](https://github.com/nordklart/nordklart/issues/243).*
...
- Works today because schema_sync trigger whitelists notes; breaks if earlier trigger version is deployed (forks, downgrades, hosted).
...
- Fix: Document hosted reverse-proxy requirement; use `CF-Connecting-IP` behind Cloudflare or platform-provided peer address.
...
- Fix: `signOut({ scope: 'global' })`; clear `nordklart-company-id` and `nordklart-invite-token` cookies with maxAge=0.
...
- BFL 5 kap 6§ requires räkenskapsinformation; nordklart treats documents as best-effort.
...
- SSRF-through-proxy DoS pattern; nordklart's Vercel egress IP shared across customers → easy to get flagged.
...
- Linear backoff `RETRY_DELAY_MS * (attempt + 1)`; may get nordklart app ID flagged by banks.
...
- File: `components/import/SIEUploadStep.tsx:97`, `BankFileUploadStep.tsx:166`, `BatchCategorySelector.tsx:56`, `NordklartMigrationWorkspace.tsx:988`, `app/(dashboard)/import/page.tsx:239`
...
- `console.log('[DEV] Company invite URL:', data.data.inviteUrl)` exposes `nordklart_inv_` token plaintext in browser console.
- Fix: Truncate token: `inviteUrl.replace(/nordklart_inv_\S+/, '[TOKEN]')` or use server-side log.
