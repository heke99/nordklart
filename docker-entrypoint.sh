...
if [ -d /opt/nordklart-template/.next ]; then
  cp -R /opt/nordklart-template/.next/. /app/.next/
fi
if [ -d /opt/nordklart-template/public ]; then
  cp -R /opt/nordklart-template/public/. /app/public/
fi
...
find /app/.next -type f \( -name '*.js' -o -name '*.html' -o -name '*.rsc' -o -name '*.meta' -o -name '*.body' \) -exec sed -i \
  ...
  -e "s|__NEXT_PUBLIC_BRANDING_APP_NAME__|${NEXT_PUBLIC_BRANDING_APP_NAME:-Nordklart}|g" \
  {} +
