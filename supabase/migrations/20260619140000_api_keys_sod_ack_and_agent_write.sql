...
-- The memory tools nordklart_remember_fact / nordklart_forget_fact were previously
...
BEGIN
  ...
  IF v_null_scope_keys > 0 THEN
    RAISE WARNING
      'agent:write grandfathering skipped % active NULL-scope key(s). These fall back to DEFAULT_SCOPES (no agent:write) and lose access to nordklart_remember_fact/nordklart_forget_fact. Review them: SELECT id, name, created_at FROM api_keys WHERE revoked_at IS NULL AND scopes IS NULL;',
      v_null_scope_keys;
  END IF;
END
$$;
