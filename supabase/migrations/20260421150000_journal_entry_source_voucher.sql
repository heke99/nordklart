...
-- on each journal entry, for per-verifikat traceability from source system → nordklart.
--
-- Context: when the SIE importer skips an empty/single-line/unbalanced voucher,
-- subsequent vouchers end up with nordklart numbers that drift from the source
