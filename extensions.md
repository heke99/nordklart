...
Nordklart is a Swedish accounting platform for sole traders (enskild firma) and limited companies (aktiebolag). It handles the legally required bookkeeping and financial management that every Swedish business needs.
...
Extensions are enabled at **build time** via `extensions.config.json`. All compiled extensions are active for all users — there is no per-user toggle system. The operator (hosted or hosted) decides which extensions to include.
...
```bash
# 1. Clone and install
git clone <repo-url> && cd Nordklart
npm install
