 ...
 * Reads JSON-RPC from stdin, POSTs to the nordklart MCP endpoint, writes response to stdout.
 *
 ...
 *     "nordklart": {
 *       "command": "node",
 *       "args": ["/path/to/nordklart/scripts/mcp-bridge.mjs"],
 *       "env": {
 *         "NORDKLART_API_KEY": "nordklart_sk_...",
 *         "NORDKLART_URL": "http://localhost:3000/api/extensions/ext/mcp-server/mcp"
 *       }
 *     }
 *   }
 * }
...
const URL = process.env.NORDKLART_URL || 'http://localhost:3000/api/extensions/ext/mcp-server/mcp'

if (!API_KEY) {
  process.stderr.write('Error: NORDKLART_API_KEY environment variable is required\n')
  process.exit(1)
}
