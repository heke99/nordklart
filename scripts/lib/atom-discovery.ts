...
export interface DiscoveredAtom {
   ...
   * into the system prompt / returned by nordklart_load_skill. (We deliberately do
   ...
   * nordklart_load_skill(<child id>) call after the parent SKILL.md is loaded.
   */
...
function buildReferencesFooter(parentId: string, refs: ReferenceFile[]): string {
  const lines = [
    ...
    '## Loadable references',
    '',
    'The reference files named above are NOT included in this body. When a question genuinely needs that depth, load the specific one on demand with `nordklart_load_skill` — and only then:',
    '',
  ]
  for (const r of refs) {
    const label = r.descriptor ? ` — ${r.descriptor}` : ''
    lines.push(`- \`${r.relPath}\` → \`nordklart_load_skill("${parentId}/${r.slug}")\`${label}`)
  }
  return '\n' + lines.join('\n') + '\n'
}
