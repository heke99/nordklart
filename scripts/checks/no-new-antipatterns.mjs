...
function findMigrationsMissingRls() {
  ...
  for (const f of files) {
    ...
    // `create table if not exists` written in prose — a comment describing what
    // another migration does — leaves the optional IF NOT EXISTS group unmatched
    // when the phrase is followed by punctuation rather than a space, and the
    // name group then captures `if`. No table can be called that without being
    // quoted, so treating these as table names only produces false offenders.
    const NOT_A_TABLE_NAME = new Set(['if', 'not', 'exists'])
    const created = new Set()
    for (const m of src.matchAll(CREATE_RE)) {
      if (!NOT_A_TABLE_NAME.has(m[1])) created.add(m[1])
    }
