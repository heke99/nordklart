/**
 * /llms.txt — agent-discoverable index of the Nordklart API.
 *
...
export async function GET(_request: Request) {
  ...
  const body = `# Nordklart API

> Swedish double-entry bookkeeping as a public REST API. API version ${API_V1_VERSION}.

This API lets agents and integrations do anything the Nordklart dashboard can do —
...
1. Create an API key in the Nordklart dashboard at /settings/api.
2. Authenticate with \`Authorization: Bearer nordklart_sk_<live|test>_<random>\`.
...
- **Test mode.** API keys prefixed \`nordklart_sk_test_\` are bound to deterministic
...
- Security disclosure policy: ${base}/SECURITY.md (responsible disclosure to security@nordklart.se)
...
\`Nordklart-Version: YYYY-MM-DD\` header for opt-in upgrades; older versions keep
