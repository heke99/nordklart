...
type ProductRow = { id: string; code: string; name: string; product_type: 'subscription' | 'addon' | 'one_time'; status: string; sort_order: number }
...
export default async function PlatformPricePlansPage({
  ...
  const [
    ...
    supabase.from('platform_plan_versions').select('id,plan_id,version_number,status,effective_from,effective_until,price_excl_vat,vat_rate,currency,billing_interval,trial_days,monthly_included_clients,grace_days,stripe_product_id,stripe_price_id,published_at').order('effective_from', { ascending: false }).order('version_number', { ascending: false }),
