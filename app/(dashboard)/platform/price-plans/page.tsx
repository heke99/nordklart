...
type PlanRow = { id: string; product_id: string; code: string; name: string; description: string | null; billing_interval: string; currency: string; status: string; trial_days: number; monthly_included_clients: number | null; sort_order: number; audience_type: string; company_form_scope: string; is_public: boolean; public_name: string | null; public_summary: string | null; public_badge: string | null; public_sort_order: number; cta_label: string | null; cta_href: string | null }
...
export default async function PlatformPricePlansPage({
  ...
  const [
    ...
    supabase.from('platform_price_plans').select('id,product_id,code,name,description,billing_interval,currency,status,trial_days,monthly_included_clients,sort_order,audience_type,company_form_scope,is_public,public_name,public_summary,public_badge,public_sort_order,cta_label,cta_href').order('sort_order', { ascending: true }),
          ...
          <details className="rounded-3xl border bg-card p-5 shadow-sm">
            ...
            <form action={createPricePlanAction} className="mt-5 space-y-5">
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <label className="text-sm font-medium">Målgrupp<select name="audience_type" defaultValue="company" className="mt-1 h-10 w-full rounded-lg border bg-background px-3"><option value="company">Företag</option><option value="agency">Redovisningsbyrå</option><option value="addon">Tillägg</option><option value="internal">Intern</option></select></label>
                <label className="text-sm font-medium">Bolagsform<select name="company_form_scope" defaultValue="company_all" className="mt-1 h-10 w-full rounded-lg border bg-background px-3"><option value="company_all">AB och enskild firma</option><option value="limited_company">Aktiebolag</option><option value="sole_trader">Enskild firma</option><option value="agency">Byrå</option><option value="not_applicable">Ej relevant</option></select></label>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><label className="text-sm font-medium">Visa publikt<select name="is_public" defaultValue="false" className="mt-1 h-10 w-full rounded-lg border bg-background px-3"><option value="false">Nej</option><option value="true">Ja</option></select></label><label className="text-sm font-medium">Publikt namn<input name="public_name" placeholder="Företag Plus" className="mt-1 h-10 w-full rounded-lg border bg-background px-3" /></label><label className="text-sm font-medium">Badge<input name="public_badge" placeholder="Populär" className="mt-1 h-10 w-full rounded-lg border bg-background px-3" /></label><label className="text-sm font-medium">Publik sortering<input name="public_sort_order" type="number" defaultValue="100" className="mt-1 h-10 w-full rounded-lg border bg-background px-3" /></label></div>
              <label className="block text-sm font-medium">Publik sammanfattning<textarea name="public_summary" rows={2} className="mt-1 w-full rounded-lg border bg-background px-3 py-2" /></label>
              ...
              return (
                <section key={plan.id} className="rounded-3xl border bg-card p-5 shadow-sm">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div><div className="flex flex-wrap items-center gap-2"><Badge variant={plan.status === 'active' ? 'success' : 'secondary'}>{plan.status}</Badge><Badge variant="secondary">{product?.product_type ?? 'okänd produkt'}</Badge><Badge variant={plan.audience_type === 'agency' ? 'warning' : 'secondary'}>{plan.audience_type === 'agency' ? 'Byrå' : plan.audience_type === 'company' ? 'Företag' : plan.audience_type}</Badge>{plan.is_public ? <Badge variant="success">Publik</Badge> : <Badge variant="secondary">Privat</Badge>}<span className="font-mono text-xs text-muted-foreground">{plan.code}</span></div><h2 className="mt-3 text-2xl font-semibold">{plan.name}</h2><p className="mt-2 max-w-2xl text-sm text-muted-foreground">{plan.description || 'Ingen beskrivning.'}</p></div>
                  ...
                  <div className="mt-5 grid gap-4 xl:grid-cols-2">
                    ...
                    <details className="rounded-2xl border bg-background/60 p-4"><summary className="cursor-pointer font-medium">Katalog, målgrupp och publik visning</summary><form action={updatePlanStatusAction} className="mt-4 grid gap-3 md:grid-cols-2"><input type="hidden" name="plan_id" value={plan.id} /><label className="text-sm">Namn<input name="name" defaultValue={plan.name} className="mt-1 h-10 w-full rounded-lg border bg-background px-3" /></label><label className="text-sm">Status<select name="status" defaultValue={plan.status} className="mt-1 h-10 w-full rounded-lg border bg-background px-3"><option value="active">Aktiv</option><option value="paused">Pausad</option><option value="archived">Arkiverad</option></select></label><label className="text-sm">Målgrupp<select name="audience_type" defaultValue={plan.audience_type ?? 'company'} className="mt-1 h-10 w-full rounded-lg border bg-background px-3"><option value="company">Företag</option><option value="agency">Redovisningsbyrå</option><option value="addon">Tillägg</option><option value="internal">Intern</option></select></label><label className="text-sm">Bolagsform<select name="company_form_scope" defaultValue={plan.company_form_scope ?? 'company_all'} className="mt-1 h-10 w-full rounded-lg border bg-background px-3"><option value="company_all">AB och enskild firma</option><option value="limited_company">Aktiebolag</option><option value="sole_trader">Enskild firma</option><option value="agency">Byrå</option><option value="not_applicable">Ej relevant</option></select></label><label className="text-sm">Visa publikt<select name="is_public" defaultValue={plan.is_public ? 'true' : 'false'} className="mt-1 h-10 w-full rounded-lg border bg-background px-3"><option value="false">Nej</option><option value="true">Ja</option></select></label><label className="text-sm">Publikt namn<input name="public_name" defaultValue={plan.public_name ?? ''} className="mt-1 h-10 w-full rounded-lg border bg-background px-3" /></label><label className="text-sm">Badge<input name="public_badge" defaultValue={plan.public_badge ?? ''} className="mt-1 h-10 w-full rounded-lg border bg-background px-3" /></label><label className="text-sm">Publik sortering<input name="public_sort_order" type="number" defaultValue={plan.public_sort_order ?? 100} className="mt-1 h-10 w-full rounded-lg border bg-background px-3" /></label><label className="text-sm md:col-span-2">Beskrivning<textarea name="description" defaultValue={plan.description ?? ''} rows={2} className="mt-1 w-full rounded-lg border bg-background px-3 py-2" /></label><label className="text-sm md:col-span-2">Publik sammanfattning<textarea name="public_summary" defaultValue={plan.public_summary ?? ''} rows={2} className="mt-1 w-full rounded-lg border bg-background px-3 py-2" /></label><input type="hidden" name="sort_order" value={plan.sort_order} /><div><Button size="sm" variant="secondary" type="submit">Spara kataloguppgifter</Button></div></form></details>
                  </div>
