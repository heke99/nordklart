...
describe('GET /api/events', () => {
  ...
  it('returns events with API key auth', async () => {
    mockExtractBearerToken.mockReturnValue('nordklart_sk_test123')
    ...
    expect(status).toBe(200)
    expect(body.data).toHaveLength(2)
    expect(mockValidateApiKey).toHaveBeenCalledWith('nordklart_sk_test123')
  })

  it('returns 401 for invalid API key', async () => {
    mockExtractBearerToken.mockReturnValue('nordklart_sk_invalid')
  ...
  it('returns 429 for rate-limited API key', async () => {
    mockExtractBearerToken.mockReturnValue('nordklart_sk_limited')
