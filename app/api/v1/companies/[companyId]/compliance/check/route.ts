 ...
 *
 * Nordklart's defensible edge: a single, structured pre-flight endpoint that
 * surfaces the same compliance checks the MCP / dashboard run, in a form
 * an agent can act on programmatically.
 *
 * Generalises the existing MCP tools (nordklart_vat_close_check,
 * nordklart_year_end_readiness) under a single response shape. New check types
...
registerEndpoint({
  ...
  pitfalls: [
    ...
    'vat_close is documented in the plan but NOT yet supported by this endpoint — call nordklart_vat_close_check via the MCP server until the function is extracted into lib/reports/.',
  ],
  example: {
    response: {
      data: {
