...
registerEndpoint({
  ...
  description:
    'Marks a draft invoice as sent — for invoices delivered outside Nordklart (Peppol, postal, manual email). Allocates the F-series invoice_number atomically (ML 17 kap 24§ p.2). On accounting_method=accrual, also posts the invoice journal entry (Debit AR 1510 / Credit revenue + output VAT). Emits invoice.sent. Idempotent and dry-runnable. The companion :send action (PR-B-2b-3) adds PDF rendering and email delivery on top of this same flow.',
  useWhen:
    'You delivered the invoice through a channel other than Nordklart\'s email (Peppol, postal, your own SMTP) and need to record it as sent so the F-series number is allocated and the journal entry is posted.',
  doNotUseFor:
    'Sending the invoice via Nordklart email — use :send (PR-B-2b-3) for that. Marking an already-sent invoice as paid — use :mark-paid (PR-B-2b-2).',
  pitfalls: [
