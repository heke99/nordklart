...
registerEndpoint({
  ...
  useWhen:
    'You need to fetch an invoice PDF for archival, forwarding to a customer outside the Nordklart send flow, or attaching to an external workflow.',
  doNotUseFor:
  ...
  pitfalls: [
    ...
    'Credit notes embed the original invoice\'s löpnummer per ML 17 kap 22–23§ — if the original was hard-deleted (not possible via Nordklart but theoretically via a manual DB edit), the reference is omitted.',
  ],
  example: {
    response: {
