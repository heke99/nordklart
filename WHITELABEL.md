...
Nordklart is whitelabel-friendly: every user-visible brand reference reads from a single `BrandingService` (`lib/branding/service.ts`). If you don't override anything, the app behaves exactly like upstream nordklart. To run your own brand on top of Nordklart, fork the repo and override the values you care about.

## Quick start

```bash
# 1. Fork nordklart/nordklart on GitHub → you/your-brand
# 2. Clone and add upstream remote (one-time)
git clone https://github.com/you/your-brand
cd your-brand
git remote add upstream https://github.com/nordklart/nordklart
...
|---|---|---|
| `NEXT_PUBLIC_BRANDING_APP_NAME` | `appName` | `Nordklart` |
| `NEXT_PUBLIC_BRANDING_APP_DESCRIPTION` | `appDescription` | `Ekonomihantering` |
| `BRANDING_LEGAL_ENTITY` | `legalEntity` | `Nordklart` |
| `BRANDING_SUPPORT_EMAIL` | `supportEmail` | `support@nordklart.se` |
| `BRANDING_PRIVACY_EMAIL` | `privacyEmail` | `privacy@nordklart.se` |
| `BRANDING_SECURITY_EMAIL` | `securityEmail` | `security@nordklart.se` |
| `NEXT_PUBLIC_BRANDING_AUTH_EMAIL_FROM` | `authEmailFrom` — From address Supabase Auth sends verification / reset emails from. Used to pre-populate the `from:` query on the "open in Gmail" button after signup. Set to whatever you configured in your Supabase Auth SMTP. | `noreply@nordklart.se` |
| `NEXT_PUBLIC_APP_URL` | `appUrl` | `https://app.nordklart.se` |
| `NEXT_PUBLIC_BRANDING_LOGO_PATH` | `logoPath` | `/nordklarticeon-removebg-preview.png` |
...
|---|---|---|
| `nordklart-company-id` | cookie | Active company context — renaming breaks logged-in sessions |
| `nordklart-invite-token` | cookie | Pre-auth invite token holding — renaming drops in-flight invites |
| `nordklart_sk_` | API key prefix | All issued API keys; existing clients fail validation |
| `nordklart_inv_` | invite token prefix | All sent invite links break |
| `nordklart_*` | MCP tool names (`nordklart_list_invoices`, etc.) | Published MCP API — Claude clients have these cached |
| `nordklart-mcp` | npm package name | Whitelabel users still install `npx nordklart-mcp`. Document `NORDKLART_URL=https://app.your-brand.se/api/extensions/ext/mcp-server/mcp` so they hit your endpoint |
| `NORDKLART_API_KEY` | env var read by `nordklart-mcp` package | Same reason — npm consumer expects this name |
...
- **iCal feed PRODID** (`lib/calendar/ics-generator.ts`) — defaults to `nordklart.se`, callers may pass their domain.
      ...
      - name: Push and open PR (clean merge)
        ...
        run: |
          ...
          gh pr create \
            ...
            --title "Sync from upstream Nordklart" \
            --body "Automated weekly sync from \`nordklart/nordklart@main\`."
