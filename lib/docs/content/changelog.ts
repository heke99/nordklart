...
> Reverse-chronological release notes for the Nordklart REST API. Versions follow Stripe's dated format (\`YYYY-MM-DD\`). The current version is **\`${API_V1_VERSION}\`**.
...
- API key auth via \`Authorization: Bearer nordklart_sk_<live|test>_<random>\`. 100 RPM rate limit per key.
- \`nordklart_sk_test_*\` keys bound to deterministic sandbox companies.
...
- **Signature**: \`X-Nordklart-Signature: t=<unix>,v1=<hex-HMAC-SHA256>\`. Stripe-format. Sample receivers in [Node + Python](/docs/api/webhooks#verifying-signatures).
