...
export function buildErrorReferenceMd(): string {
  ...
  lines.push('# Errors')
  lines.push('')
  lines.push(`> Every error returned by the Nordklart REST API uses a stable code from this catalogue. Codes never change once shipped — agents can pattern-match on them safely. The \`docs_url\` field on every error envelope points at the anchor for that specific code.`)
  lines.push('')
  ...
  lines.push('    "docs_url": "https://app.nordklart.se/docs/api/errors#period_locked"')
  lines.push('  },')
