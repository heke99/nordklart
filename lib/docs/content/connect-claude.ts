...
> Talk to your bookkeeping. Connect Nordklart to Claude (claude.ai, Claude Desktop, or Claude Code) and ask questions, categorise transactions, and prepare a momsdeklaration in plain language — every write still stages for your approval first.

Nordklart ships an [MCP](https://modelcontextprotocol.io) server that exposes the full bookkeeping engine — 90+ tools — to any MCP client. The endpoint is:

\`\`\`
https://app.nordklart.se/api/extensions/ext/mcp-server/mcp
\`\`\`
...
Best for most users. No API key to manage — you authorise Nordklart the same way you'd authorise any OAuth app.
...
2. Paste the connector URL:
   ...
   https://app.nordklart.se/api/extensions/ext/mcp-server/mcp
   \`\`\`
3. Claude opens the Nordklart OAuth 2.1 consent screen. Sign in and pick the company you want Claude to act on.
...
5. Approve. Claude now lists the Nordklart tools and you can start asking questions.
...
## Path B — \`npx nordklart-mcp\` with an API key (stdio bridge)

Best for Claude Desktop on a machine where you'd rather use a long-lived API key than the OAuth flow, or for scripting.

1. Mint an API key in the Nordklart dashboard at **/settings/api**. Use a \`nordklart_sk_test_*\` key against the sandbox while you evaluate; switch to \`nordklart_sk_live_*\` for real data.
...
All three run against the deterministic sandbox seed (use a \`nordklart_sk_test_*\` key or pick the sandbox company on the OAuth consent screen). They exercise the read path end-to-end without booking anything.

1. **"Show my uncategorized bank transactions and suggest categories."**
   Claude calls \`nordklart_list_uncategorized_transactions\` then \`nordklart_suggest_categories\` and walks you through the proposals. Approving one stages a \`nordklart_categorize_transaction\` pending operation — nothing is booked until you confirm.
2. **"Which invoices are overdue?"**
   Claude calls \`nordklart_get_ar_ledger\` (kundreskontra) and lists outstanding customer invoices with aging.
3. **"Compute my VAT report for this quarter and tell me if I can close it."**
   Claude calls \`nordklart_get_vat_report\` for the momsdeklaration rutor, then \`nordklart_vat_close_check\` to scan for blockers (uncategorised rows, unapproved supplier invoices, missing receipts on expenses ≥ 4 000 kr — the tool's high-value heuristic; BFL requires underlag for every affärshändelse regardless of amount) and reports \`ready_to_close\`.
...
1. **Connect.** Use Path A (read-only scopes only) or Path B with a \`nordklart_sk_test_*\` key. → Claude lists the Nordklart tools (titles like *List Uncategorized Transactions*, *VAT Declaration (Momsdeklaration)*).
