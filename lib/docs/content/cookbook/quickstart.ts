...
- A test API key (\`nordklart_sk_test_*\`) from the Nordklart dashboard at **/settings/api**. Test keys are bound to a deterministic sandbox company seeded with realistic data — safe for evals.
...
\`\`\`bash
curl https://app.nordklart.se/api/v1/companies \\
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/customers?dry_run=true" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
...
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/invoices" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/invoices/$INVOICE_ID/send" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)"
\`\`\`
...
\`\`\`json
{
  ...
  "meta": {
    ...
    "audit": {
      ...
      "voucher_url": "https://app.nordklart.se/bookkeeping/...",
...
- **Test keys can't email real addresses.** \`nordklart_sk_test_*\` short-circuits external providers — \`/send\` returns success but no email goes out. The PDF is still generated and the voucher posted.
