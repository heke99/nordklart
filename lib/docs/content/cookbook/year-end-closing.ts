...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/reports/continuity-check?from=2025-01-01&to=2025-12-31" \\
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/compliance/check?type=voucher_gaps&period=2025" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/compliance/check?type=unmatched_documents&period=2025" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/fiscal-periods/$PERIOD_ID/lock" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/fiscal-periods/$PERIOD_ID/year-end" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)" \\
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/reports/trial-balance?period=2026" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/fiscal-periods/$PERIOD_ID/close" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)" \\
  -H "Content-Type: application/json" \\
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/reports/annual-report?year=2025" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
# Aktiebolag — INK2
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/reports/ink2?year=2025" \\
  -H "Authorization: Bearer nordklart_sk_test_..."

# Enskild firma — NE-bilaga
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/reports/ne-bilaga?year=2025" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
- **Periodiseringsfond reserve cap.** Per IL 30 kap 5 § and 30 kap 6 a §, AB can set aside max 25% of the **taxable profit AFTER schablonintäkt has been added back and BEFORE the periodiseringsfond deduction itself**. The schablonintäkt is **SLR × the periodiseringsfonder at the start of the tax year** (IL 30 kap 6 a §, floor 0.5%), where SLR is statslåneräntan on 30 November of the year before the tax year ends — **1.96% for 2025 and 2.55% for 2026**. The engine reads the canonical rate from \`tax_rates\` and surfaces both the schablonintäkt amount and the resulting cap on the year-end result block; pass \`to_periodiseringsfond\` as the desired set-aside amount and the engine returns \`VALIDATION_ERROR\` with the maximum allowed value if it exceeds the cap. Note: under BFL/BFNAR 2016:10 kap 13 (materiellt samband for AB), periodiseringsfond is BOOKED as an obeskattad reserv on accounts 2110–2139, not just declared on INK2 — the engine posts the booking automatically as part of the year-end procedure.
