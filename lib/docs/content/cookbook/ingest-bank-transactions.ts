...
> Push a bank statement file into Nordklart, get AI-assisted category suggestions, commit the categorisations, and match payments against open invoices. End-to-end transaction-to-booking pipeline.
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/imports/bank" \\
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/operations/$OPERATION_ID" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/transactions?status=uncategorized&period=2026-04&limit=50" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/transactions/$TX_ID/suggest-categories" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/transactions/$TX_ID/categorize?dry_run=true" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)" \\
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/transactions/batch-categorize" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)" \\
  -H "Content-Type: application/json" \\
  -d '{
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/transactions/$TX_ID/match-invoice" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)" \\
  -H "Content-Type: application/json" \\
