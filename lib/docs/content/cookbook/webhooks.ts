...
- A receiver URL that Nordklart can POST to. For local development use [smee.io](https://smee.io) or \`ngrok\` — Nordklart refuses webhook URLs that resolve to private IPs (SSRF protection), so localhost won't work directly.
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/webhooks" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)" \\
  -H "Content-Type: application/json" \\
  -d '{
    "event_type": "invoice.paid",
    "webhook_url": "https://my-receiver.example.com/nordklart",
...
Use the [Node](https://app.nordklart.se/docs/api/webhooks#nodejs) or [Python](https://app.nordklart.se/docs/api/webhooks#python) sample on the concept page. The critical detail: capture the **raw request body** before any framework JSON-parses it. Re-serialising the body produces different bytes and the signature won't match.
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/webhooks/$WEBHOOK_ID/test" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`
POST /nordklart HTTP/1.1
Content-Type: application/json
X-Nordklart-Signature: t=1715797800,v1=2f5c...
X-Nordklart-Event: webhook.test
X-Nordklart-Delivery: wh_dlv_...
X-Nordklart-Api-Version: 2026-05-12

{"id":"wh_dlv_...","type":"webhook.test","api_version":"2026-05-12","created":1715797800,"data":{"object":{"hello":"from Nordklart","tested_at":"2026-05-15T12:00:00Z"}},"previous_attributes":null}
\`\`\`
...
\`\`\`bash
curl "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/webhooks/$WEBHOOK_ID/deliveries?delivery_id=$DELIVERY_ID" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/invoices/$INVOICE_ID/mark-paid" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Idempotency-Key: $(uuidgen)" \\
  -H "Content-Type: application/json" \\
...
Deliveries are at-least-once. The same \`X-Nordklart-Delivery\` may arrive twice when the network drops a 200 response or your receiver times out after processing. Build idempotency around that header:
...
This pattern: a unique constraint on \`delivery_id\`, an INSERT-on-conflict-do-nothing, and short-circuit when nothing was inserted. Every Nordklart delivery passes through that gate at most once even if the dispatcher retries.
...
\`\`\`bash
curl -X POST "https://app.nordklart.se/api/v1/webhook-deliveries/$DELIVERY_ID/retry" \\
  -H "Authorization: Bearer nordklart_sk_test_..."
\`\`\`

The retry creates a fresh delivery row pointing at the same payload — the original audit row stays in place. Receivers see the same \`X-Nordklart-Delivery\` (the new row's id, not the original's), so the idempotency table needs no special handling.
...
\`\`\`bash
curl -X PATCH "https://app.nordklart.se/api/v1/companies/$COMPANY_ID/webhooks/$WEBHOOK_ID" \\
  -H "Authorization: Bearer nordklart_sk_test_..." \\
  -H "Content-Type: application/json" \\
  -d '{ "active": true }'
\`\`\`
...
- **Re-serialising the body.** \`JSON.parse(rawBody); JSON.stringify(parsed)\` produces different bytes than Nordklart sent. Always sign-check against the raw bytes.
