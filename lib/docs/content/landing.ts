...
curl https://app.nordklart.se/api/v1/companies \\
  -H "Authorization: Bearer nordklart_sk_live_..."
\`\`\`

Create keys in the nordklart dashboard at **/settings/api**. Two key prefixes are available:

- \`nordklart_sk_live_*\` — hits real customer data. Use in production.
- \`nordklart_sk_test_*\` — bound to deterministic sandbox companies. Safe for evals, demos, and agent learning. Same surface, different blast radius.
...
\`\`\`json
{
  "error": {
    ...
    "docs_url": "https://app.nordklart.se/docs/api/errors#period_locked"
  },
...
- **[\`/.well-known/skills/index.json\`](/.well-known/skills/index.json)** — nordklart-specific skill catalogue.
`
