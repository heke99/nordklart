...
> Receive HMAC-signed POST notifications when state changes in Nordklart — invoices paid, journal entries committed, periods locked, salary runs booked, AGI files generated. At-least-once delivery with exponential backoff over ~72 hours.

If you've used [Stripe webhooks](https://docs.stripe.com/webhooks), the model is identical: subscribe a URL to an event type, Nordklart POSTs each event with a signed JSON body, your receiver returns 2xx to acknowledge. The signature header format and retry policy are the same. The event types are nordklart-specific.
...
2. **Nordklart emits events** internally (e.g. an invoice is marked paid via the dashboard or another API call). The webhook handler enqueues a delivery row.
...
User-Agent: nordklart-webhook/1
X-Nordklart-Signature: t=1715797800,v1=2f5c...
X-Nordklart-Event: invoice.paid
X-Nordklart-Delivery: wh_dlv_a8f1...
X-Nordklart-Api-Version: 2026-05-12
X-Request-Id: whdel_a8f1...
\`\`\`

The \`X-Nordklart-Delivery\` header is the canonical correlation id — log it on receipt and use it to deduplicate retries (deliveries are at-least-once, so the same delivery id may arrive more than once after a network blip).
...
// is computed against the exact bytes Nordklart sent, not a re-serialised JSON.
app.post(
  '/webhook',
  express.raw({ type: 'application/json' }),
  (req, res) => {
    const sigHeader = req.header('x-nordklart-signature') ?? ''
...
def webhook():
    ...
    sig_header = request.headers.get("X-Nordklart-Signature", "")
...
- **Using parsed JSON instead of raw bytes.** Re-serialising the body (\`JSON.stringify(req.body)\`) produces different bytes than Nordklart sent — the signature won't match. Capture the raw body before any framework parses it.
...
- **Treating retries as duplicates of failure.** Retries arrive when *we* didn't get a 2xx. A 200 response that arrives slowly may not reach us in time and we'll retry — your receiver sees the same \`X-Nordklart-Delivery\` twice. Idempotency is on you.
...
To replay a \`dead\` or \`delivered\` delivery, call [\`POST /api/v1/webhook-deliveries/{deliveryId}/retry\`](/docs/api/reference/webhooks#post-retry). The retry creates a fresh delivery row pointing at the same payload — the original audit row stays in place. Receivers must be idempotent on the \`X-Nordklart-Delivery\` header.
...
For accounting-event delivery rows (\`journal_entry.*\`, \`period.*\`, \`salary_run.booked\`, \`agi.generated\`, \`invoice.paid\`, \`supplier_invoice.paid\`), Nordklart keeps the delivery rows for 7 years. **This is a voluntary operational audit-trail policy Nordklart chose because the duration aligns conveniently with BFL 7 kap retention on the underlying records — it is NOT itself a statutory obligation.** The 7-year statutory retention under BFL 7 kap 1 § applies to the underlying verifikation / faktura / AGI XML in its own table, not to the delivery envelope. The integrator's own retention obligations likewise attach to the underlying records you receive (and any local copies you persist), not to the delivery-row metadata.
