...
\`\`\`
Nordklart-Version: ${API_V1_VERSION}
\`\`\`
\`\`\`json
...
API requests pin per-request via the \`Nordklart-Version\` request header (planned for v1.x; today every request gets the current version):

\`\`\`bash
curl https://app.nordklart.se/api/v1/companies \\
  -H "Authorization: Bearer ..." \\
  -H "Nordklart-Version: ${API_V1_VERSION}"
\`\`\`
...
4. Three months before retirement, every response from a deprecated version stamps \`Nordklart-Deprecation: <ISO date>\` in headers.
...
\`\`\`bash
curl https://app.nordklart.se/api/v1/companies/{cid}/invoices \\
  -H "Authorization: Bearer ..." \\
...
\`\`\`json
{
  ...
  "meta": {
    ...
    "audit": {
      ...
      "audit_trail_url": "https://app.nordklart.se/audit/req_...",
