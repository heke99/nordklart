  ...
  capture: async ({ transaction_id }, { supabase, companyId }) => {
    ...
    for (const d of docCandidates) {
      ...
      if (!ex) {
        ...
        // suggest calling nordklart_get_document_content directly.
        underlag.push({
          kind: 'receipt',
          document_id: d.id,
          merchant_name: null,
  ...
  promptTemplate: ({ captured, profileSummary }) => {
    ...
    if (captured.underlag.length === 0) {
      ...
      for (const u of captured.underlag) {
        ...
        lines.push(`  • ${u.kind}: ${parts.join(', ') || '(ingen extraherad data — läs underlaget med nordklart_get_document_content)'}`)
      }
      lines.push('')
    ...
    lines.push(`- När du är säker, staga via nordklart_categorize_transaction med transaction_id=${tx.id} (ALDRIG document_id). Välj kategori från enum-listan i verktygets schema.`)
