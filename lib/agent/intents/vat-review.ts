...
export const vatReview = defineAgentIntent<VatReviewArgs, CapturedVatReview>({
  ...
  promptTemplate: ({ captured, profileSummary }) => {
    ...
    lines.push('1. Hämta declarationen med nordklart_get_vat_report (period_type, year, period).')
    lines.push('2. Hämta nordklart_vat_close_check för pre-filing varningar (t.ex. ensidig reverse charge utan motpost).')
