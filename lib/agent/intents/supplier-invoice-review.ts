...
>({
  ...
  tools: [
    ...
    'nordklart_approve_supplier_invoice',
    'nordklart_credit_supplier_invoice',
  ...
  promptTemplate: ({ captured, profileSummary }) => {
    ...
    if (ex) {
      ...
      lines.push('Underlag är bifogat men inte AI-extraherat. Använd nordklart_get_document_content för att läsa PDF/bilden om du behöver fler signaler.')
      lines.push('')
    } else {
    ...
    lines.push('4. Föreslå rätt BAS-konto för varje rad (eller för hela fakturan om bara en summarad finns). Följ leverantörens historik — nordklart_query_journal({ text: "<leverantörens namn>", source_type: "supplier_invoice", limit: 5 }) — när du väljer konto.')
    lines.push('5. Om du är säker, staga attestering via nordklart_approve_supplier_invoice. Annars: peka på vad som ska klargöras innan attestering.')
    lines.push('')
