...
import type { SheetSpec } from '@/lib/reports/xlsx-export'
 ...
 */
export async function buildRegisterExport<TRow>(
  spec: ReadonlyArray<SheetSpec<TRow>>,
  ...
  if (format === 'csv') {
    const buf = await reportToWorkbook(spec, { bookType: 'csv' })
    return {
  ...
  return {
    buffer: await reportToWorkbook(spec),
