...
describe('generateApiKey', () => {
  it('returns key starting with "nordklart_sk_"', () => {
    const { key } = generateApiKey()
    expect(key.startsWith('nordklart_sk_')).toBe(true)
  })
  ...
  it('returns prefix of KEY_PREFIX + 8 chars', () => {
    ...
    expect(prefix).toBe(key.slice(0, 'nordklart_sk_'.length + 8))
  })
...
describe('hashApiKey', () => {
  it('returns 64-char hex string', () => {
    const hash = hashApiKey('nordklart_sk_test-key')
    expect(hash).toMatch(/^[0-9a-f]{64}$/)
  })

  it('is deterministic for same input', () => {
    const hash1 = hashApiKey('nordklart_sk_deterministic')
    const hash2 = hashApiKey('nordklart_sk_deterministic')
    expect(hash1).toBe(hash2)
  })

  it('produces different hashes for different inputs', () => {
    const hash1 = hashApiKey('nordklart_sk_key-a')
    const hash2 = hashApiKey('nordklart_sk_key-b')
...
describe('extractBearerToken', () => {
  ...
  it('handles token with special characters', () => {
    const request = new Request('http://localhost', {
      headers: { authorization: 'Bearer nordklart_sk_abc+def/ghi=jkl' },
    })
    expect(extractBearerToken(request)).toBe('nordklart_sk_abc+def/ghi=jkl')
  })
})
...
describe('agent:write scope', () => {
  ...
  it('keeps nordklart_get_agent_briefing on agent:read', () => {
    expect(TOOL_SCOPE_MAP.nordklart_get_agent_briefing).toBe('agent:read')
  })
...
describe('validateApiKey', () => {
  ...
  it('rejects keys not starting with "nordklart_sk_"', async () => {
  ...
  it('rejects a refresh token presented as Bearer with a specific message', async () => {
    const result = await validateApiKey('nordklart_rt_some_refresh_token')
  ...
  it('rejects when RPC returns error', async () => {
    ...
    const result = await validateApiKey('nordklart_sk_test-key-value')
