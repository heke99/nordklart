...
const FROM = 'noreply@nordklart.se'
...
describe('detectWebmailHint', () => {
  it('returns Gmail with a pre-populated search for gmail.com', () => {
    ...
    expect(hint!.url).toBe(
      `https://mail.google.com/mail/u/0/#search/${encodeURIComponent('from:noreply@nordklart.se')}`,
    )
  })
  ...
  it('URL-encodes the sender address (handles special characters)', () => {
    const hint = detectWebmailHint('user@gmail.com', 'no+reply@nordklart.se')
    expect(hint!.url).toContain(encodeURIComponent('from:no+reply@nordklart.se'))
  })
})
