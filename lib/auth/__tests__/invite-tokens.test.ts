...
describe('generateInviteToken', () => {
  it('returns token starting with "nordklart_inv_"', () => {
    const { token } = generateInviteToken()
    expect(token.startsWith('nordklart_inv_')).toBe(true)
  })
...
describe('hashInviteToken', () => {
  it('returns 64-char hex string', () => {
    const hash = hashInviteToken('nordklart_inv_test-token')
    expect(hash).toMatch(/^[0-9a-f]{64}$/)
  })

  it('is deterministic for same input', () => {
    const hash1 = hashInviteToken('nordklart_inv_deterministic')
    const hash2 = hashInviteToken('nordklart_inv_deterministic')
