...
//     linked entry's status. Honors nordklart.allow_delete (20260506140000).
...
//     linked entry is posted/reversed. Also honors nordklart.allow_delete.
//
...
describe('document-immutability.pg — BFL retention bypass guards', () => {
  ...
  it('respects nordklart.allow_delete bypass — delete_last_voucher RPC keeps working', async () => {
    const { userId, companyId, fiscalPeriodId } = await seedCompany()
    const entryId = await insertEntryAtStatus({
      userId, companyId, fiscalPeriodId, voucherNumber: 1,
    })
...
// linked to posted entries unless the nordklart.allow_supersede GUC is set —
...
describe('document-immutability.pg — version supersession on posted entries', () => {
  ...
  it('keeps blocking sha256_hash mutation even when allow_supersede is set', async () => {
    ...
    try {
      await client.query('BEGIN')
      await client.query(`SELECT set_config('nordklart.allow_supersede', 'true', true)`)
      await expect(
        client.query(
  ...
  it('respects nordklart.allow_supersede bypass on direct UPDATE', async () => {
    const { userId, companyId, fiscalPeriodId } = await seedCompany()
    const entryId = await insertEntryAtStatus({
      userId, companyId, fiscalPeriodId, voucherNumber: 1,
    })
