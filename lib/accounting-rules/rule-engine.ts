...
export function evaluatePurchaseForAccounting(input: PurchaseRuleInput, context: EntityContext): RuleDecision {
  ...
  const vatDeductible = Math.round((vat.deductiblePercentage * deductibility.deductiblePercentage) / 100)
  ...
  if (assetCandidate && assetCandidate.decision === 'review_required') {
    return {
      ...
      requiredEvidence: [...deductibility.requiredEvidence, ...propertyEvidence(assetCandidate.reasonCode)],
  ...
  return {
    ...
    requiredEvidence: deductibility.requiredEvidence,
    reviewSeverity: maxSeverity(deductibility.reviewSeverity, assetCandidate?.reviewSeverity ?? 'none', vat.warnings.length ? 'warning' : 'none'),
    warnings,
    suggestedAsset: assetCandidate?.suggestedAsset,
  }
}
...
function propertyEvidence(reasonCode: string) {
  ...
  return [
    { code: 'land_building_split', labelSv: 'fördelning mellan mark, byggnad och markanläggning' },
    { code: 'property_action_type', labelSv: 'bedömning av reparation, förbättring eller komponentbyte' },
  ]
}

function maxSeverity(...levels: RuleDecision['reviewSeverity'][]): RuleDecision['reviewSeverity'] {
  ...
  return levels.reduce((max, level) => (order[level] > order[max] ? level : max), 'none')
}
