...
describe('getStructuredError', () => {
  ...
  it('extracts code from typed error class with code property', () => {
    ...
    expect(result.remediation?.resource).toBe('Nordklart://chart-of-accounts')
  })
  ...
  it('infers PERIOD_NOT_LOCKED from message text', () => {
    ...
    expect(result.remediation?.tool).toBe('nordklart_lock_period')
  })
  ...
  it('infers PERIOD_HAS_UNBOOKED_TRANSACTIONS from Swedish lock-error message', () => {
    ...
    expect(result.remediation?.tool).toBe('nordklart_list_uncategorized_transactions')
  })
  ...
  it('produces INSUFFICIENT_SCOPE remediation with attempted scope', () => {
    ...
    expect(result.remediation?.resource).toBe('Nordklart://capabilities')
  })
  ...
  it('infers TRANSACTION_ALREADY_CATEGORIZED', () => {
    ...
    expect(result.remediation?.tool).toBe('nordklart_uncategorize_transaction')
  })
