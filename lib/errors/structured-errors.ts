...
const GENERIC: Record<string, StructuredErrorEntry> = {
  ...
  INSUFFICIENT_SCOPE: {
    ...
    remediation: {
      ...
      resource: 'Nordklart://capabilities',
    },
  },
}
...
const BOOKKEEPING: Record<string, StructuredErrorEntry> = {
  ACCOUNTS_NOT_IN_CHART: {
    ...
    remediation: {
      ...
      resource: 'Nordklart://chart-of-accounts',
    },
  },
  ...
  FISCAL_PERIOD_NOT_FOUND: {
    ...
    remediation: {
      ...
      resource: 'Nordklart://period/active',
    },
  },
  ...
  PERIOD_LOCKED: {
    ...
    remediation: {
      description:
        'Either unlock the period via nordklart_unlock_period (if status is "locked", not "closed") or change the entry date to fall inside an open period.',
      tool: 'nordklart_unlock_period',
    },
  },
  PERIOD_NOT_LOCKED: {
    ...
    remediation: {
      description: 'Call nordklart_lock_period before closing.',
  ...
  TRANSACTION_ALREADY_CATEGORIZED: {
    ...
    remediation: {
      description:
        'Use nordklart_uncategorize_transaction first if you need to recategorize.',
      tool: 'nordklart_uncategorize_transaction',
    },
  },
...
const LINK_INVOICE_VOUCHER: Record<string, StructuredErrorEntry> = {
  ...
  LINK_VOUCHER_NO_AR_CREDIT: {
    httpStatus: 400,
    message_sv:
      'Verifikationen krediterar inte ett kundfordringskonto (151x). Bokföringen behöver först rättas med en stornoverifikation som krediterar 1510, t.ex. via nordklart_correct_entry.',
    message_en:
      'The journal entry does not credit an accounts-receivable account (151x). Correct the booking first via a storno+correction (nordklart_correct_entry) that credits 1510.',
    remediation: {
      description:
        'Use nordklart_correct_entry to storno the existing voucher and re-book the receipt as Dr 1930 / Cr 1510, then link the corrected voucher.',
      tool: 'nordklart_correct_entry',
    },
  },
  ...
  LINK_VOUCHER_AMOUNT_EXCEEDS_REMAINING: {
    ...
    message_en:
      'The voucher\'s AR credit exceeds the invoice\'s remaining balance. Split the voucher across multiple invoices via nordklart_correct_entry first, or pick a different voucher.',
  },
...
const LINK_SI_VOUCHER: Record<string, StructuredErrorEntry> = {
  ...
  LINK_SI_VOUCHER_NO_AP_DEBIT: {
    httpStatus: 400,
    message_sv:
      'Verifikationen debiterar inget leverantörsskuldskonto (244x). Rätta bokföringen först med en stornoverifikation som debiterar t.ex. 2440 (SEK) eller 2441 (utländsk valuta), via nordklart_correct_entry.',
    message_en:
      'The journal entry does not debit any accounts-payable account in the 244x range (e.g. 2440 SEK, 2441 foreign currency). Correct the booking first via a storno+correction (nordklart_correct_entry).',
    remediation: {
      description:
        'Use nordklart_correct_entry to storno the existing voucher and re-book the payment as Dr 244x / Cr 1930, then link the corrected voucher.',
      tool: 'nordklart_correct_entry',
    },
  },
  ...
  LINK_SI_VOUCHER_AMOUNT_EXCEEDS_REMAINING: {
    ...
    message_en:
      'The voucher\'s AP debit exceeds the supplier invoice\'s remaining balance. Split the voucher across multiple supplier invoices via nordklart_correct_entry first, or pick a different voucher.',
  },
