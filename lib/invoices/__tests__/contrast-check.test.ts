...
describe('isWcagAACompliant', () => {
  ...
  // Nordklart brand semantic colors. All three are used as DATA-ONLY indicators
  ...
  it('Nordklart terracotta (#c2410c equivalent dark red) passes AA on white', () => {
    // 5.91:1 — passes AA for normal text.
    expect(isWcagAACompliant('#c2410c', '#ffffff')).toBe(true)
  })

  it('Nordklart sage (#84a98c lighter green) fails AA on white', () => {
    // ~2.4:1 — fails AA, as expected for a soft pastel sage.
    expect(isWcagAACompliant('#84a98c', '#ffffff')).toBe(false)
  })

  it('Nordklart ochre (#d4a373 warm yellow) fails AA on white', () => {
