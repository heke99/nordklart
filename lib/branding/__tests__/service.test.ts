...
describe('branding service', () => {
  ...
  it('returns nordklart defaults when nothing is overridden', async () => {
    ...
    expect(b.authEmailFrom).toBe('noreply@nordklart.se')
    expect(b.logoPath).toBe('/nordklart-icon.png')
  ...
  it('empty string env var does not override', async () => {
    ...
    const { getBranding } = await import('../service')
    expect(getBranding().appName).toBe('Nordklart')
  })
