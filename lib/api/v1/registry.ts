...
export function generateOpenApiSpec(serverUrl: string): OpenApiSpec {
  ...
  return {
    ...
    info: {
      ...
      description:
        'Public REST API for Nordklart — Swedish double-entry bookkeeping. ' +
        'Every write supports dry-run via `?dry_run=true`. Every request must include ' +
        '`Authorization: Bearer nordklart_sk_...`. See /docs/api for the cookbook.',
    },
    ...
    components: {
      securitySchemes: {
        [SCHEME_NAME]: {
          type: 'http',
          scheme: 'bearer',
          bearerFormat: 'nordklart_sk_<live|test>_<random>',
        },
      },
    },
