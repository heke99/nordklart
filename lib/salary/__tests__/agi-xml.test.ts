...
describe('generateAGIXml — Avsandare', () => {
  it('includes program name "nordklart"', () => {
    const xml = generateAGIXml(company, employees, totals)
    expect(xml).toContain('<gem:Programnamn>nordklart</gem:Programnamn>')
  })
...
describe('generateAGIXml — Frånvarouppgift', () => {
  ...
  it('emits FK820/824/826 absence-removal and percent fields not at all (Nordklart always sends timmar)', () => {
    const xml = generateAGIXml(company, employeesWithAbsence, totals)
