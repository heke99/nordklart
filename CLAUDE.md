...
**Path alias**: `@/*` maps to the project root. **Language**: All code, comments, and commit messages in English. **License**: Proprietary.
...
**API keys** (`lib/auth/api-keys.ts`): SHA-256 hashed, `nordklart_sk_` prefix. Scoped via `TOOL_SCOPE_MAP`. Rate limited 100 RPM via `validate_and_increment_api_key` RPC.
...
**Enabled** (`extensions.config.json`): `enable-banking` (PSD2), `email` (Resend), `nordklart-migration`, `tic` (org lookup), `mcp-server`, `cloud-backup` (Google Drive), `skatteverket`, `invoice-inbox`, `document-extraction`. **Present but disabled**: `calendar`, `push-notifications`, `example-logger` (plus the `_example-branding` template).
...
Nordklart exposes its bookkeeping engine as an MCP server (`extensions/general/mcp-server/`) for Claude Desktop/Code — 90+ tools, JSON-RPC 2.0, endpoint `/api/extensions/ext/mcp-server/mcp`, OAuth 2.1 for Claude connectors. npm bridge: `packages/nordklart-mcp` (`npx nordklart-mcp`).

**API keys** (`lib/auth/api-keys.ts`, `api_keys` table): SHA-256, `nordklart_sk_` prefix, scoped via `TOOL_SCOPE_MAP`, 100 RPM via `validate_and_increment_api_key` RPC. `createServiceClientNoCookies()` — all queries filter by `company_id` (defense in depth).
...
- **Docker (hosted)**: 4-stage Node 22 Alpine `Dockerfile` (standalone output) + `docker-compose.yml` (app + supercronic cron). `docker-entrypoint.sh` validates env vars and replaces build-time placeholders in `.next/static/`. Extension presets: `docker/extensions.{hosted,hosted}.json`.
...
Never create a NUL/nul file: `\Nordklart\NUL`.
