# Hosted Deployment Nordklart

This guide walks you through deploying Nordklart on your own infrastructure using Docker.
...
1. Set **Site URL** to your deployment URL (e.g., `https://nordklart.example.com`).
2. Add `https://nordklart.example.com/auth/callback` to the **Redirect URLs** allowlist.

Nordklart uses email + password authentication with magic link as a fallback. The default Supabase email auth settings work out of the box. For production, configure a custom SMTP provider under **Authentication > SMTP Settings** to avoid Supabase's built-in rate limits.

MFA (two-factor authentication via TOTP) is **not enforced** for hosted deployments — the Docker image sets `NEXT_PUBLIC_SELF_HOSTED=true` by default, which disables MFA enforcement. Users can still optionally enable 2FA in Settings > Säkerhet if they wish.
...
| Container | Purpose |
|-----------|---------|
| `app` | Next.js application (`ghcr.io/nordklart/nordklart:latest`) |
...
The hosted Docker image includes these AI-powered extensions: receipt OCR, AI categorization, AI chat, and invoice inbox. To enable them, add API keys to your `.env`:
...
Check the [release notes](https://github.com/nordklart/nordklart/releases) for migration instructions.
