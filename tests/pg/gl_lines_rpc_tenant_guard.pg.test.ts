...
async function insertPostedJournalEntry(params: {
  ...
  await getPool().query(
    `WITH e AS (
     ...
     SELECT id, '1930', $7, 0 FROM e
     UNION ALL
     SELECT id, '2091', 0, $7 FROM e`,
    [id, params.userId, params.companyId, params.fiscalPeriodId, params.voucherNumber, params.entryDate, amount],
  )
  return id
}
