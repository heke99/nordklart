 ...
 *
 * Degrades silently when no email service is registered (e.g. hosted
...
export async function handleSkattekontoDriftDetected(
  ...
  const appUrl = (process.env.NEXT_PUBLIC_APP_URL || 'https://nordklart.se').replace(/\/$/, '')
  ...
  const lines = [
    ...
    '',
    'Logga in på Nordklart för att se beloppen och granska skattekonto-raderna:',
    dashboardLink,
    '',
...
<p><a href="${escapeHtml(dashboardLink)}">Logga in på Nordklart</a> för att se beloppen och granska skattekonto-raderna.</p>
