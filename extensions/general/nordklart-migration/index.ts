...
import type { NordklartProvider } from './types'
import { NORDKLART_PROVIDERS } from './types'
 ...
 * (Fortnox, Visma, Bokio, Björn Lundén, Briox) into Nordklart by talking
 ...
 */
export const nordklartMigrationExtension: Extension = {
  id: 'nordklart-migration',
  ...
  apiRoutes: [
    // ── List available providers ───────────────────────────────────
    {
      ...
      path: '/providers',
      handler: async () => {
        return NextResponse.json({ providers: NORDKLART_PROVIDERS })
      },
    },
    ...
    // ── Check existing connections and import history ──────────────
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        try {
          ...
          moduleLog.error('nordklart status failed', error as Error, { companyId })
    ...
    // ── Start consent flow (create consent + OTC) ─────────────────
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        const providerInfo = NORDKLART_PROVIDERS.find(p => p.id === provider)
        if (!providerInfo) {
        ...
        try {
          ...
          log.error('nordklart connect failed', error as Error, { provider })
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        const respondWithError = (reason: string) => {
          ...
          const html = `<!DOCTYPE html><html><body><script>
            if (window.opener) {
              window.opener.postMessage({ type: 'nordklart-oauth-error', reason: ${jsLiteral(reason)} }, ${jsLiteral(appUrl)});
              window.close();
            } else {
        ...
        try {
          ...
          let provider: NordklartProvider | null = null
          ...
          try {
            ...
            if (decoded.consentId && decoded.provider) {
              ...
              provider = decoded.provider as NordklartProvider
            }
          } catch {
            // Legacy fallback
          }
          ...
          if (!consentId || !provider) {
            ...
            provider = ctx?.settings
              ? await ctx.settings.get<NordklartProvider>('provider')
              : null
          }
          ...
          const redirectUri = `${appUrl}/api/extensions/ext/nordklart-migration/callback`
          ...
          const html = `<!DOCTYPE html><html><body><script>
            if (window.opener) {
              window.opener.postMessage({ type: 'nordklart-oauth-success', consentId: ${jsLiteral(consentId)} }, ${jsLiteral(appUrl)});
              window.close();
            } else {
    ...
    // ── Preview: fetch company info + SIE stats before migration ──
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        try {
          ...
          log.error('nordklart preview failed', error as Error)
    ...
    // ── Fetch + parse SIE data for mapping step ───────────────────
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        try {
          ...
          log.error('nordklart sie-data fetch failed', error as Error)
          const classified = classifyProviderError(error)
    ...
    // ── Import SIE data (accounts, balances, vouchers) ────────────
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        try {
          ...
          log.error('nordklart sie import failed', error as Error)
          const classified = classifyProviderError(error)
    ...
    // without writing.
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        try {
          ...
          log.info('nordklart reconcile completed', {
            companyId,
            dryRun,
          ...
          log.error('nordklart reconcile failed', error as Error)
    ...
    // ── Accept consent (mark as fully connected after import) ─────
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        try {
          ...
          moduleLog.error('nordklart accept failed', error as Error, { consentId })
    ...
    // ── Disconnect / revoke consent ───────────────────────────────
    {
      ...
      handler: async (request: Request, ctx?: ExtensionContext) => {
        ...
        try {
          ...
          log.error('nordklart disconnect failed', error as Error, { consentId })
