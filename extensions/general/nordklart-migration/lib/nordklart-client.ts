/**
 * HTTP client for the Nordklart Sync gateway API.
 *
...
function getBaseUrl(): string {
  const url = process.env.NORDKLART_SYNC_GATEWAY_URL
  if (!url) throw new Error('NORDKLART_SYNC_GATEWAY_URL is not configured')
  return url.replace(/\/$/, '')
}

function getApiKey(): string {
  const key = process.env.NORDKLART_SYNC_API_KEY
  if (!key) throw new Error('NORDKLART_SYNC_API_KEY is not configured')
  return key
}
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (attempt < MAX_RETRIES && isAbort) {
        console.warn(`[nordklart] ${path} timed out, retrying (attempt ${attempt + 1})`)
        await new Promise(r => setTimeout(r, RETRY_DELAY_MS * (attempt + 1)))
        continue
      }
      if (isAbort) {
        throw new Error(`Nordklart API timeout after ${Math.round(timeoutMs / 1000)}s: ${path}`)
      }
      throw err
    } finally {
      clearTimeout(timer)
    ...
    if (attempt < MAX_RETRIES && RETRYABLE_STATUSES.includes(response.status)) {
      console.warn(`[nordklart] ${path} returned ${response.status}, retrying (attempt ${attempt + 1})`)
      await new Promise(r => setTimeout(r, RETRY_DELAY_MS * (attempt + 1)))
      continue
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`Nordklart API ${response.status}: ${body || response.statusText}`)
    }

    return response.json()
  }

  throw new Error(`Nordklart API failed after ${MAX_RETRIES + 1} attempts: ${path}`)
}
