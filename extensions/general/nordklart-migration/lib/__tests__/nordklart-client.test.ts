...
vi.stubEnv('NORDKLART_SYNC_GATEWAY_URL', 'https://nordklart.test.com')
vi.stubEnv('NORDKLART_SYNC_API_KEY', 'test-api-key')

import { getConsent, createConsent, fetchCompanyInfo, fetchCustomers } from '../nordklart-client'

describe('nordklart-client', () => {
  ...
  describe('retry', () => {
    ...
    it('throws after exhausting all retries on retryable status', async () => {
      ...
      await expect(getConsent('c1')).rejects.toThrow('Nordklart API 503')
    ...
    it('does not retry on 400 errors', async () => {
      ...
      await expect(getConsent('c1')).rejects.toThrow('Nordklart API 400')
      expect(fetchSpy).toHaveBeenCalledTimes(1)
    })
    ...
    it('does not retry on 404 errors', async () => {
      ...
      await expect(getConsent('c1')).rejects.toThrow('Nordklart API 404')
      expect(fetchSpy).toHaveBeenCalledTimes(1)
    })
  })
  ...
  describe('timeout retry', () => {
    ...
    it('throws timeout error after exhausting retries on AbortError', async () => {
      ...
      await expect(getConsent('c1')).rejects.toThrow('Nordklart API timeout')
      expect(fetchSpy).toHaveBeenCalledTimes(3)
    })
  ...
  describe('request body', () => {
    it('sends JSON body for POST requests', async () => {
      ...
      expect(url).toBe('https://nordklart.test.com/api/v1/consents')
  ...
  describe('environment validation', () => {
    it('throws when NORDKLART_SYNC_GATEWAY_URL is missing', async () => {
      const orig = process.env.NORDKLART_SYNC_GATEWAY_URL
      delete process.env.NORDKLART_SYNC_GATEWAY_URL

      try {
        await expect(getConsent('c1')).rejects.toThrow(
          'NORDKLART_SYNC_GATEWAY_URL is not configured'
        )
      } finally {
        process.env.NORDKLART_SYNC_GATEWAY_URL = orig
      }
    })

    it('throws when NORDKLART_SYNC_API_KEY is missing', async () => {
      const orig = process.env.NORDKLART_SYNC_API_KEY
      delete process.env.NORDKLART_SYNC_API_KEY

      try {
        await expect(getConsent('c1')).rejects.toThrow(
          'NORDKLART_SYNC_API_KEY is not configured'
        )
      } finally {
        process.env.NORDKLART_SYNC_API_KEY = orig
      }
    })
  })
})
