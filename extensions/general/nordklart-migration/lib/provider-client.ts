...
import { encryptProviderToken } from '@/lib/providers/token-encryption'
...
export async function exchangeAuthToken(
  ...
  // Store tokens (encrypted at rest when PROVIDER_TOKEN_ENCRYPTION_KEY is set)
  await supabase
    .from('provider_consent_tokens')
    .upsert({
      consent_id: consentId,
      provider,
      access_token: encryptProviderToken(tokenResponse.access_token),
...
export async function submitProviderToken(
  ...
  // Store tokens — consent stays at status 0 until migration/SIE import
  // completes. Encrypted at rest when PROVIDER_TOKEN_ENCRYPTION_KEY is set.
  await supabase
    .from('provider_consent_tokens')
    .upsert({
      consent_id: consentId,
      ...
      refresh_token: refreshToken ? encryptProviderToken(refreshToken) : refreshToken,
