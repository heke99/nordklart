/**
 * Maps Nordklart Sync canonical DTOs to Nordklart internal types.
 *
 * These mappers transform the normalized data from any Swedish accounting
 * provider into the exact shapes Nordklart expects for database insertion.
 */
...
export function mapSalesInvoice(
  ...
  // Map Nordklart status to Nordklart status
  const statusMap: Record<string, string> = {
    draft: 'draft',
    sent: 'sent',
    booked: 'sent', // Nordklart has no 'booked' status — treat as sent
    paid: 'paid',
    overdue: 'overdue',
    cancelled: 'cancelled',
    credited: 'credited',
...
export function mapSupplierInvoice(
  ...
  if (isCreditNote) {
    ...
    // nordklart gateway is the only source of invoiceTypeCode and is NOT guaranteed
