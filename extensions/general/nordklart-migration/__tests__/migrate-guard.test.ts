...
import { nordklartMigrationExtension } from '../index'
import { executeMigration } from '../lib/migration-orchestrator'
import { getConsent } from '../lib/provider-client'

const migrateRoute = (nordklartMigrationExtension.apiRoutes ?? []).find(
...
function migrateRequest() {
  return createMockRequest('http://localhost/api/extensions/ext/nordklart-migration/migrate', {
    method: 'POST',
