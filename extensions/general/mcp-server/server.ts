...
// nordklart_feedback rate limit: 1 per 60s per actor. In-memory single-process;
// no Redis dependency. See the nordklart_feedback tool definition below.
...
function buildApprovalGuidance(operationId: string, riskLevel: 'low' | 'medium' | 'high'): string {
  if (riskLevel === 'high') {
    return `This is an irreversible posting under BFL 5 kap 5§ — surface the irreversibility implications to the user and obtain an explicit acknowledgment before committing. Once the user has acknowledged, call nordklart_approve_pending_operation with operation_id="${operationId}" and confirmed=true.`
  }
  return `When the user authorises, call nordklart_approve_pending_operation with operation_id="${operationId}".`
}
...
async function stagePendingOperation(
  ...
  if (options.idempotencyKey && requestHash) {
    ...
    if (cached) {
      ...
      return {
        ...
        ...(cachedOpId
          ? { approve: { tool: 'nordklart_approve_pending_operation', args: { operation_id: cachedOpId } } }
          : {}),
  ...
  // warning dialog). The server-side check in nordklart_approve_pending_operation
  ...
  const response = {
    ...
    approve: {
      tool: 'nordklart_approve_pending_operation',
...
async function categorizeTransactionCore(
  ...
  if (transaction.journal_entry_id) {
    return {
      ...
      journal_entry_error: 'Transaction already has a journal entry — use nordklart_list_uncategorized_transactions to find unbooked ones.',
      category,
      debit_account: '',
      credit_account: '',
...
// ── VAT report computation (shared by nordklart_get_vat_report + nordklart_vat_review_widget) ──
//
 ...
 *  or extend the list — Nordklart's BAS chart only ships 3001/3002/3003/3004
...
// Cheap, embedding-free near-duplicate detection for nordklart_remember_fact.
...
export async function computeVatCloseCheck(
  ...
  if (uncategorizedCount > 0) {
    blockers.push({
      ...
      hint: 'Kategorisera via nordklart_categorize_transaction eller kör nordklart_auto_match_period.',
    })
  }
  ...
  if (unapprovedCount > 0) {
    blockers.push({
      ...
      hint: 'Attestera via nordklart_approve_supplier_invoice — ingående moms (ruta 48) påverkas.',
    })
  }
  if (!reconRes.is_reconciled) {
    blockers.push({
      ...
      hint: 'Granska via nordklart_get_reconciliation_status och matcha — moms beräknas från huvudboken så differenser döljer fel.',
    })
  }
  ...
  if (missingReceipts > 0) {
    blockers.push({
      ...
      hint: 'BFL 5 kap 6§: varje affärshändelse måste ha verifikat. Använd nordklart_list_unmatched_documents för att para ihop.',
    })
  }
...
export const tools: McpTool[] = [
  {
    name: 'nordklart_search_tools',
    title: 'Search MCP Tools',
    description: 'Search Nordklart MCP tools by keyword and return their schemas at a chosen detail level. Call this first when looking for a capability — avoids loading every tool schema upfront.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
    ...
    async execute(args, _companyId, _userId, _supabase, _actor) {
      ...
      //
      // The dispatcher injects __keyScopes when it routes to nordklart_search_tools.
      ...
      if (query) {
        ...
        const ranked = candidates
          .map((t, idx) => {
            ...
            let score = 0
            if (name === query || name === `nordklart_${query}` || name.endsWith(`_${query}`)) score += 100
  ...
  {
    name: 'nordklart_list_skills',
    title: 'List Domain Skills',
    description: 'List available domain-knowledge skills filtered to this company (entity type, VAT, payroll). Workflow guides + loaded specialty atoms. Pass include_all=true to see hidden skills. Call nordklart_load_skill(slug) for any body.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_load_skill',
    title: 'Load Domain Skill',
    description: 'Load a skill body by slug. Workflow slugs are flat (e.g. "month-end-close"); atom slugs match registry ids (e.g. "vertical/konsult-it", "modifier/holding-ab"). Call nordklart_list_skills to find slugs.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
...

  {
    ...
    inputSchema: {
      ...
      additionalProperties: false,
      properties: {
        id: { type: 'string', description: 'The memory entry id from a prior nordklart_remember_fact call.' },
...

  {
    ...
    async execute(args, companyId, userId, _supabase, actor) {
      ...
      if (last && now - last < FEEDBACK_RATE_LIMIT_MS) {
        ...
        throw new Error(`nordklart_feedback is rate-limited. Try again in ${waitSec}s.`)
      }
  ...
  {
    name: 'nordklart_get_agent_briefing',
    title: 'Get Agent Briefing',
    description: 'Bootstrap this company\'s specialized accountant context in one call: profile_summary, the atoms loaded for the company (metadata only — call nordklart_load_skill for bodies), and the top-30 active memories. Call once at session start.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {},
    ...
    outputSchema: {
      ...
      properties: {
        ...
        atoms: {
          type: 'array',
          description: 'Atoms (horizontal/vertical/modifier skills) loaded for this company. Metadata only — call nordklart_load_skill(id) for the body.',
          items: {
            type: 'object',
            additionalProperties: false,
            properties: {
              id: { type: 'string', description: 'Atom id (e.g. "horizontal/swedish-vat", "vertical/konsult-it", "modifier/holding-ab"). Use as nordklart_load_skill slug.' },
  ...
  {
    name: 'nordklart_create_transactions',
    title: 'Create Bank Transactions',
    description: 'Stage one or more transactions for the user to approve. Each creates a separate pending operation; commit each via nordklart_approve_pending_operation. Use for ingesting external rows (Airtable, CSV). Max 10.',
    outputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const staged = await stagePendingOperation(
          ...
          actor,
          {
            description: 'Once approved, the transaction lands in /transactions as uncategorized. Use nordklart_categorize_transaction to book it.',
            tool: 'nordklart_categorize_transaction',
          },
  ...
  {
    name: 'nordklart_list_uncategorized_transactions',
  ...
  {
    name: 'nordklart_list_transactions_without_documents',
    title: 'List Transactions Missing Receipts',
    description: 'List BANK TRANSACTIONS booked without an attached underlag. For imported/manual verifikat (no bank tx row) call nordklart_list_verifikat_without_documents — this tool only covers bank-driven entries.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_list_verifikat_without_documents',
  ...
  {
    name: 'nordklart_categorize_transaction',
    title: 'Categorize Bank Transaction',
    description: 'Categorize a bank transaction. Stages the journal entry; commit via nordklart_approve_pending_operation. If an underlag is attached it rejects vat_treatment="reverse_charge" when the seller already charged VAT.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      const result = await categorizeTransactionCore(
        ...
        companyId,
        supabase,
        false // preview mode — execution happens at approval time via nordklart_approve_pending_operation
      )
      ...
      return stagePendingOperation(supabase, companyId, userId, 'categorize_transaction',
        ...
        actor,
        {
          description: 'Once approved, the journal entry is posted. Continue with nordklart_list_uncategorized_transactions to keep clearing the backlog, or lock the period once it is empty.',
          tool: 'nordklart_list_uncategorized_transactions',
        },
  ...
  {
    name: 'nordklart_receipt_matcher',
  ...
  {
    name: 'nordklart_create_customer',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'create_customer',
        ...
        actor,
        {
          description: 'Once approved, you can invoice this customer with nordklart_create_invoice using the returned customer_id.',
          tool: 'nordklart_create_invoice',
        },
        {
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      if (!customerId) throw new Error('customer_id is required. Use nordklart_list_customers to find IDs.')
      ...
      if (custError || !customer) {
        throw new Error('Customer not found. Use nordklart_list_customers to find valid IDs.')
      }
      ...
      return stagePendingOperation(supabase, companyId, userId, 'create_invoice',
        ...
        actor,
        {
          description: 'Once approved, the invoice is created as a draft. Send it with nordklart_send_invoice or use nordklart_mark_invoice_as_sent if delivered outside the system.',
          tool: 'nordklart_send_invoice',
        }
      )
    },
  },
  ...
  {
    name: 'nordklart_get_trial_balance',
...

  {
    ...
    // report+widget surface; nordklart_vat_review_widget remains as an alias.
  ...
  {
    name: 'nordklart_vat_review_widget',
    title: 'VAT Review Widget',
    description: 'Open the interactive VAT review widget for a period. Equivalent to nordklart_get_vat_report(render_ui=true); kept as an alias for clients pinned to this tool name.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_vat_close_check',
  ...
  {
    name: 'nordklart_get_income_statement',
  ...
  {
    name: 'nordklart_mark_invoice_as_paid',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'mark_invoice_paid',
        ...
        actor,
        {
          description: 'Once approved, the payment is booked (15xx → 19xx). Use nordklart_get_ar_ledger to confirm the customer balance reflects it.',
          tool: 'nordklart_get_ar_ledger',
        },
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'send_invoice',
        ...
        actor,
        {
          description: 'After the customer pays, mark the invoice paid via nordklart_mark_invoice_as_paid (or match it to the inbound bank transaction with nordklart_match_transaction_to_invoice).',
          tool: 'nordklart_mark_invoice_as_paid',
  ...
  {
    name: 'nordklart_mark_invoice_as_sent',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'mark_invoice_sent',
        ...
        actor,
        {
          description: 'Once approved, the invoice moves to "sent". Track its payment via nordklart_mark_invoice_as_paid when the customer pays.',
          tool: 'nordklart_mark_invoice_as_paid',
          args: { invoice_id: invoiceId },
        }
      )
  ...
  {
    name: 'nordklart_create_supplier',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'create_supplier',
        ...
        actor,
        {
          description: 'Once approved, you can book supplier invoices against this supplier with nordklart_create_supplier_invoice_from_inbox using the returned supplier_id.',
          tool: 'nordklart_create_supplier_invoice_from_inbox',
        },
        {
  ...
  {
    name: 'nordklart_list_supplier_invoices',
  ...
  {
    name: 'nordklart_get_counterparty_templates',
  ...
  {
    name: 'nordklart_suggest_categories',
  ...
  {
    name: 'nordklart_get_balance_sheet',
  ...
  {
    name: 'nordklart_get_general_ledger',
    title: 'General Ledger (Huvudbok)',
    description: 'General ledger (huvudbok) for a fiscal period: per-account opening balance, entries, closing balance. Optional account range filter. For ad-hoc cross-account, amount, or free-text line queries use nordklart_query_journal.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_get_supplier_ledger',
  ...
  {
    name: 'nordklart_match_transaction_to_invoice',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'match_transaction_invoice',
        ...
        actor,
        {
          description: 'After approval the transaction is linked and the invoice is marked paid. Use nordklart_get_ar_ledger to verify the customer balance.',
          tool: 'nordklart_get_ar_ledger',
        }
      )
    },
  ...
  {
    name: 'nordklart_match_batch_allocate',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'match_batch_allocate',
        ...
        // nordklart_link_transaction_to_journal_entry — it's the minimum
        ...
        actor,
        {
          description: 'After approval the combined verifikat is created and each invoice is advanced. Verify with nordklart_get_ar_ledger (customer) or nordklart_get_supplier_ledger.',
          tool: hasCustomer ? 'nordklart_get_ar_ledger' : 'nordklart_get_supplier_ledger',
        },
  ...
  {
    name: 'nordklart_link_transaction_to_journal_entry',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      // logic in nordklart_bulk_book_transactions.
      ...
      return stagePendingOperation(
        ...
        actor,
        {
          description: invoiceId
            ? 'After approval the tx attaches to the existing verifikat and the invoice flips to paid/partially_paid. No new bokföring. Verify with nordklart_get_ar_ledger.'
            : 'After approval the tx attaches to the existing verifikat. No new bokföring. Verify with nordklart_query_journal.',
          tool: invoiceId ? 'nordklart_get_ar_ledger' : 'nordklart_query_journal',
        },
  ...
  {
    name: 'nordklart_bulk_book_transactions',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      // the explicit zero-amount guard in nordklart_match_batch_allocate.
      ...
      // Cross-currency batches should go through nordklart_match_batch_allocate
      ...
      if (currencies.size > 1) {
        ...
        throw new Error(
          'All transactions must share the same currency. For cross-currency allocations, use nordklart_match_batch_allocate (which handles the FX diff on 7960/3960).'
        )
      }
      ...
      return stagePendingOperation(supabase, companyId, userId, 'bulk_book_transactions',
        ...
        // design rationale as nordklart_link_transaction_to_journal_entry.
        {
        ...
        actor,
        {
          description: 'After approval the verifikat carries the combined business event. Verify with nordklart_query_journal or nordklart_get_reconciliation_status.',
          tool: 'nordklart_query_journal',
        },
        { dateForPeriodCheck: periodCheckDate }
      )
    },
  ...
  {
    name: 'nordklart_find_voucher_candidates_for_invoice',
    title: 'Find Voucher Candidates (Invoice)',
    description: 'List posted verifikat that credit kundfordran (1510) and could be the payment for this invoice. Use before nordklart_link_invoice_to_voucher when the user wants to mark a faktura paid against an existing verifikation (no new bokföring).',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_link_invoice_to_voucher',
    title: 'Link Invoice to Voucher',
    description: 'Markera en faktura som betald genom att länka till en befintlig verifikation som redan krediterar kundfordran (1510). Ingen ny verifikation skapas. Hitta kandidater med nordklart_find_voucher_candidates_for_invoice först.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_find_voucher_candidates_for_supplier_invoice',
    title: 'Find Voucher Candidates (Supplier Invoice)',
    description: 'List posted verifikat that debit leverantörsskuld (2440) and could be the payment for this supplier invoice. Use before nordklart_link_supplier_invoice_to_voucher when marking a leverantörsfaktura paid against an existing verifikation (no new bokföring).',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_link_supplier_invoice_to_voucher',
    title: 'Link Supplier Invoice to Voucher',
    description: 'Markera en leverantörsfaktura som betald genom att länka till en befintlig verifikation som redan debiterar leverantörsskuld (2440). Ingen ny verifikation skapas. Hitta kandidater med nordklart_find_voucher_candidates_for_supplier_invoice först.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(
        ...
        actor,
        {
          ...
          tool: 'nordklart_get_supplier_ledger',
        },
      )
    },
  },
  ...
  {
    name: 'nordklart_auto_match_period',
  ...
  {
    name: 'nordklart_list_fiscal_periods',
  ...
  {
    name: 'nordklart_get_reconciliation_status',
  ...
  {
    name: 'nordklart_upload_document',
  ...
  {
    name: 'nordklart_list_inbox_items',
  ...
  {
    name: 'nordklart_create_supplier_invoice_from_inbox',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(
        ...
        actor,
        {
          description: 'After approval, attest via nordklart_approve_supplier_invoice and pay via the bank flow.',
          tool: 'nordklart_get_inbox_item',
  ...
  {
    name: 'nordklart_list_unmatched_documents',
  ...
  {
    name: 'nordklart_get_document_content',
    title: 'Get Document Content',
    description: 'Get a 5-minute signed download URL for a document so the agent can read its contents (e.g. with vision). Use after nordklart_list_unmatched_documents to inspect a specific PDF before deciding which transaction it matches.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_attach_document_to_transaction',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(
        ...
        actor,
        {
          description: 'Once approved, the receipt is linked to the transaction. If the transaction is still uncategorized, follow up with nordklart_categorize_transaction.',
          tool: 'nordklart_categorize_transaction',
  ...
  {
    name: 'nordklart_get_salary_journal',
  ...
  {
    name: 'nordklart_create_salary_run',
    title: 'Create Salary Run',
    description: 'Stage creation of a draft salary run for a period + base lines for all active employees. Commit via nordklart_approve_pending_operation; then run nordklart_calculate_salary_run. Final booking happens in the web UI.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(
        ...
        actor,
        {
          ...
          tool: 'nordklart_calculate_salary_run',
        },
  ...
  {
    name: 'nordklart_calculate_salary_run',
  ...
  {
    name: 'nordklart_generate_agi',
    title: 'Generate AGI Declaration',
    description: 'Stage AGI XML generation (Arbetsgivardeklaration) for a salary run. High-risk: produces statutory Skatteverket underlag (BFL 7-year retention). Commit via nordklart_approve_pending_operation.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_vat_declaration_validate',
  ...
  {
    name: 'nordklart_vat_declaration_submit',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(
        ...
        actor,
        {
          description: 'After approval, sign in Skatteverket via the returned BankID link, then poll nordklart_vat_declaration_status.',
          tool: 'nordklart_vat_declaration_status',
  ...
  {
    name: 'nordklart_vat_declaration_status',
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      const { data: decl } = await supabase
        ...
        .maybeSingle()
      if (!decl?.xml_content) {
        throw new Error('AGI-underlag saknas — generera AGI först med nordklart_generate_agi.')
      }
      const period = `${run.period_year}-${String(run.period_month).padStart(2, '0')}`
      return stagePendingOperation(
        ...
        actor,
        {
          description: 'After approval, sign via the returned BankID link, then poll nordklart_agi_status.',
          tool: 'nordklart_agi_status',
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      if (!period.locked_at) throw new Error('Period must be locked before closing — call nordklart_lock_period first')
      ...
      return stagePendingOperation(supabase, companyId, userId, 'close_period',
        ...
        actor,
        {
          ...
          tool: 'nordklart_get_balance_sheet',
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'lock_period',
        ...
        actor,
        {
          description: 'After locking, run year-end closing before the period can be closed via nordklart_close_period. Verify balances first with nordklart_get_trial_balance.',
          tool: 'nordklart_get_trial_balance',
          args: { fiscal_period_id: fiscalPeriodId },
        }
      )
    },
  ...
  {
    name: 'nordklart_uncategorize_transaction',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'uncategorize_transaction',
        ...
        actor,
        {
          description: 'After approval the transaction is uncategorized again — book it with the correct category via nordklart_categorize_transaction.',
          tool: 'nordklart_categorize_transaction',
          args: { transaction_id: transactionId },
        }
      )
  ...
  {
    name: 'nordklart_year_end_readiness',
    title: 'Year-End Readiness Check',
    description: "Pre-flight before irreversible nordklart_run_year_end. Returns ready (bool) + ordered blockers (drafts, voucher gaps, sequence mismatches, unbalanced TB, FX revaluation) + optional closing-entry preview.",
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'run_year_end',
        ...
        actor,
        {
          description: 'After year-end, the period is locked and ready for closing via nordklart_close_period.',
          tool: 'nordklart_close_period',
          args: { fiscal_period_id: fiscalPeriodId },
        }
      )
    },
  ...
  {
    name: 'nordklart_set_opening_balances',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'set_opening_balances',
        ...
        actor,
        {
          description: 'After approval, verify the opening balance matches the closed period\'s UB via nordklart_get_trial_balance on the next period.',
          tool: 'nordklart_get_trial_balance',
  ...
  {
    name: 'nordklart_run_currency_revaluation',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'run_currency_revaluation',
        ...
        actor,
        {
          description: 'After approval, confirm the new FX-adjusted balances via nordklart_get_balance_sheet.',
          tool: 'nordklart_get_balance_sheet',
          args: { fiscal_period_id: fiscalPeriodId },
        }
      )
  ...
  {
    name: 'nordklart_list_voucher_gaps',
  ...
  {
    name: 'nordklart_explain_voucher_gap',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'explain_voucher_gap',
        ...
        actor,
        {
          description: 'After approval, run nordklart_list_voucher_gaps again to confirm all gaps in the period now have explanations (BFNAR 2013:2).',
          tool: 'nordklart_list_voucher_gaps',
  ...
  {
    name: 'nordklart_approve_supplier_invoice',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'approve_supplier_invoice',
        ...
        actor,
        {
          description: 'After approval the invoice is attested and ready for payment. When paid, match the outbound bank transaction via nordklart_match_transaction_to_invoice.',
          tool: 'nordklart_get_supplier_ledger',
        },
  ...
  {
    name: 'nordklart_credit_supplier_invoice',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'credit_supplier_invoice',
        ...
        actor,
        {
          description: 'After approval the credit note is posted and the leverantörsskuld cleared. Verify with nordklart_get_supplier_ledger.',
          tool: 'nordklart_get_supplier_ledger',
        }
      )
    },
  ...
  {
    name: 'nordklart_convert_invoice',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'convert_invoice',
        ...
        actor,
        {
          description: 'After conversion, send the new invoice with nordklart_send_invoice.',
          tool: 'nordklart_send_invoice',
        }
      )
    },
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'unlock_period',
        ...
        actor,
        {
          description: 'After approval, post the rättelse via nordklart_correct_entry or new entries via nordklart_create_voucher, then re-lock with nordklart_lock_period.',
          tool: 'nordklart_lock_period',
          args: { fiscal_period_id: fiscalPeriodId },
        }
      )
    },
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'import_sie',
        ...
        actor,
        {
          description: 'After commit, verify the imported balances with nordklart_get_trial_balance and check continuity via the IB/UB of adjacent periods.',
          tool: 'nordklart_get_trial_balance',
        }
      )
    },
  ...
  {
    name: 'nordklart_undo_sie_import',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(supabase, companyId, userId, 'undo_sie_import',
        ...
        actor,
        {
          description: 'After commit, re-stage the SIE import with corrected mappings via nordklart_import_sie.',
          tool: 'nordklart_import_sie',
        },
      )
    },
  },
...

  {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      if (inboxItemId) {
        ...
        if (inbox.created_journal_entry_id) {
          throw new Error(
            ...
            'Use nordklart_correct_entry or nordklart_reverse_entry if it needs to be changed.'
          )
        }
        if (inbox.created_supplier_invoice_id) {
          throw new Error(
      ...
      return stagePendingOperation(supabase, companyId, userId, 'create_voucher',
        ...
        actor,
        {
          description: 'After commit, confirm the new verifikation lands on the right accounts with nordklart_get_general_ledger or nordklart_query_journal.',
          tool: 'nordklart_query_journal',
        },
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      if (!original) {
        throw new Error(
          ...
          `If this UUID came from an earlier tool result, re-fetch via nordklart_query_journal — ` +
      ...
      return stagePendingOperation(supabase, companyId, userId, 'correct_entry',
        ...
        actor,
        {
          description: 'After commit, the original is marked reversed and a corrected verifikation lands in its place. Confirm both with nordklart_query_journal.',
          tool: 'nordklart_query_journal',
        },
  ...
  {
    name: 'nordklart_reverse_journal_entry',
    title: 'Reverse Journal Entry (Storno)',
    description: 'Stage a storno: inverts debits/credits; original stays visible per BFL 5 kap. Use only when the affärshändelse should never have been booked (duplicate, ghost, test). If booked wrong, use nordklart_correct_entry; for refunds, nordklart_credit_invoice. HIGH risk.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      // omprövning (rättelse-omprövning per ML 2023:200, SFL 22 kap). Nordklart
      ...
      return stagePendingOperation(supabase, companyId, userId, 'reverse_entry',
        ...
        actor,
        {
          description: 'After commit, the storno is posted and the original stays visible. Confirm with nordklart_query_journal.',
          tool: 'nordklart_query_journal',
        },
        { dateForPeriodCheck: original.entry_date },
      )
  ...
  {
    name: 'nordklart_propose_dispositioner',
  ...
  {
    name: 'nordklart_propose_accruals',
  ...
  {
    name: 'nordklart_propose_annual_depreciation',
  ...
  {
    name: 'nordklart_post_annual_depreciation',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      return stagePendingOperation(
        ...
        actor,
        {
          description: 'After approval, depreciation entries are posted. Continue the year-end flow via nordklart_year_end_readiness, then nordklart_run_year_end.',
          tool: 'nordklart_year_end_readiness',
          args: { fiscal_period_id: fiscalPeriodId },
        },
  ...
  {
    name: 'nordklart_preview_arsredovisning',
  ...
  {
    name: 'nordklart_preview_ef_declaration',
  ...
  {
    name: 'nordklart_list_pending_operations',
    title: 'List Pending Operations',
    description: 'List staged pending_operations. Filter by status (default pending), risk_level, or operation_type. Use to review the queue before calling nordklart_approve_pending_operation or nordklart_reject_pending_operation.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'nordklart_approve_pending_operation',
    ...
    async execute(args, companyId, userId, supabase, actor) {
      ...
      // OAuth connector's access_token is itself a minted nordklart_sk_ key
  ...
  {
    name: 'nordklart_reject_pending_operation',
  ...
  {
    name: 'nordklart_set_inbox_extracted_data',
    title: 'Set Inbox Extracted Data',
    description: 'Replace extracted_data on an inbox item with agent-supplied fields (bring-your-own-extraction). Use when your own pipeline parses the document better than Nordklart\'s OCR. Follow with nordklart_create_supplier_invoice_from_inbox to stage.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
...
/**
 * Fire-and-forget telemetry for every successful nordklart_load_skill, all tiers.
...
export async function handleMcpRequest(request: Request): Promise<Response> {
  ...
  switch (method) {
    case 'initialize': {
      ...
      return NextResponse.json(
        jsonRpc(id ?? null, {
          ...
          instructions: [
            'Nordklart — Swedish double-entry bookkeeping via conversation.',
            '',
            'Discovery:',
            '• tools/list returns the full schema for every tool. To narrow a large catalog, call nordklart_search_tools(query="…") — it ranks tools by relevance; pass detail="name"|"summary"|"full" to control payload size.',
            '• When the user asks "how do I do X" or you\'re unsure of the correct sequence (month-end close, VAT review, year-end, invoicing, payroll), call nordklart_list_skills first — domain workflows are documented as loadable skills with tool references.',
            '',
            'Common workflows:',
            '• Categorize transactions: nordklart_list_uncategorized_transactions → nordklart_suggest_categories → nordklart_categorize_transaction (stages) → nordklart_approve_pending_operation (after user confirms in chat).',
            '• Applying income to invoices — pick by what you have: a specific bank transaction + a known invoice → nordklart_match_transaction_to_invoice; an invoice you know is paid but no specific bank line → nordklart_mark_invoice_as_paid; a whole period of unmatched income to reconcile → nordklart_auto_match_period (dry_run first). All stage for approval.',
            '• Invoicing: nordklart_list_customers (or nordklart_create_customer) → nordklart_create_invoice → nordklart_send_invoice or nordklart_mark_invoice_as_sent → nordklart_mark_invoice_as_paid. Refund via nordklart_credit_invoice.',
            '• Suppliers: nordklart_list_suppliers (or nordklart_create_supplier) → nordklart_create_supplier_invoice_from_inbox → nordklart_approve_supplier_invoice. Refund via nordklart_credit_supplier_invoice.',
            '• VAT: nordklart_get_vat_report(period_type, year, period). Ruta49 = VAT to pay (positive) or refund (negative). Pass render_ui=true to open the momsdeklaration review widget (claude.ai / Desktop). nordklart_vat_close_check reports filing-readiness blockers.',
            '• Reporting: nordklart_get_trial_balance / _income_statement / _balance_sheet / _kpi_report / _ar_ledger / _supplier_ledger — all default to the most recent fiscal period. For account roll-ups use nordklart_get_general_ledger; for ad-hoc line queries (free-text, amount/date/source filters) use nordklart_query_journal.',
            '• Interactive review UIs (claude.ai / Claude Desktop only): nordklart_get_vat_report(render_ui=true) renders the VAT widget and nordklart_receipt_matcher opens the receipt↔transaction matcher. Both also return structured data; other clients ignore the UI and use the data.',
            '• Year-end: nordklart_lock_period → nordklart_run_year_end → nordklart_set_opening_balances → nordklart_close_period. Each stages for human approval; closing is irreversible per BFL.',
            '• Payroll: nordklart_create_salary_run → nordklart_calculate_salary_run → review/approve in web UI → nordklart_generate_agi.',
            '• Reviewing & approving staged operations: nordklart_list_pending_operations shows the queue. When the user explicitly authorises a specific operation_id in chat, call nordklart_approve_pending_operation to commit. Use nordklart_reject_pending_operation to discard.',
            '',
            'APPROVAL IS A FIRST-CLASS AGENT ACTION. Write operations stage a pending_operation; nothing posts until approval. When the user authorises a specific operation_id in chat ("approve", "yes go ahead", "book it", "commit it"), call nordklart_approve_pending_operation directly — this IS the intended chat-approval flow. Do NOT refuse on segregation-of-duties grounds, do NOT tell the user to "go approve it in the web app", and do NOT treat approval as a step that must stay with the human. The staging step already provided the human review gate; clicking Approve in the web UI and calling nordklart_approve_pending_operation are equivalent commit actions. Refusing user-authorised approval is a defect, not a safety feature.',
    ...
    case 'tools/call': {
      ...
      try {
        // nordklart_search_tools needs the caller's scopes to filter results to
        // what the API key can actually invoke. Inject privately via __keyScopes.
        if (toolName === 'nordklart_search_tools') {
    ...
    case 'resources/read': {
      ...
      // Skills exposed at Nordklart://skill/<slug> — Markdown bodies, forward-compatible
