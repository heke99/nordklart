...
vi.mock('@/lib/auth/api-keys', async (importOriginal) => {
  ...
  return {
    ...
    validateApiKey: vi.fn().mockResolvedValue({
      userId: 'user-1',
      companyId: 'company-1',
      // Only reports:read — enough to call nordklart_get_trial_balance, NOT enough
      // to call nordklart_create_invoice (invoices:write). Drives the scope-denied test.
      scopes: ['reports:read'],
      apiKeyId: 'key-1',
      apiKeyName: 'Test Key',
    }),
    // Minimal supabase mock — agent_atom_registry resolves to empty so
    // nordklart_list_skills happy-path doesn't crash on its registry query.
...
describe('mcp.tool_called telemetry', () => {
  ...
  it('emits on successful tool execution with success=true and a measured latencyMs', async () => {
    ...
    // nordklart_list_skills is unscoped + has no DB dependency, perfect for a happy-path test.
    const response = await handleMcpRequest(
      mcpRequest('tools/call', { name: 'nordklart_list_skills', arguments: {} })
    )
    const json = await response.json()
    expect(json.error).toBeUndefined()

    const event = await eventPromise
    expect(event.tool).toBe('nordklart_list_skills')
  ...
  it('emits errorKind=scope_denied when the API key lacks the required scope', async () => {
    ...
    // nordklart_create_invoice requires invoices:write; our test key has only reports:read.
    ...
    const event = await eventPromise
    expect(event.tool).toBe('nordklart_create_invoice')
  ...
  it('emits errorKind=unknown_tool when the tool name does not exist', async () => {
    ...
    await handleMcpRequest(
      mcpRequest('tools/call', { name: 'nordklart_does_not_exist', arguments: {} })
    )

    const event = await eventPromise
    expect(event.tool).toBe('nordklart_does_not_exist')
    ...
    expect(event.errorMessage).toBe('Unknown tool: "nordklart_does_not_exist"')
    expect(event.latencyMs).toBe(0)
  })
  ...
  it('emits errorKind=execution when the tool throws inside execute()', async () => {
    ...
    // nordklart_load_skill throws on unknown slug — clean way to force an
    ...
    const event = await eventPromise
    expect(event.tool).toBe('nordklart_load_skill')
    expect(event.success).toBe(false)
    expect(event.isError).toBe(true)
...
describe('mcp.resource_read telemetry', () => {
  ...
  it('emits kind=skill for a skill URI hit', async () => {
    ...
    await handleMcpRequest(
      mcpRequest('resources/read', { uri: 'Nordklart://skill/quarterly-vat-review' })
    )

    const event = await eventPromise
    expect(event.uri).toBe('Nordklart://skill/quarterly-vat-review')
  ...
  it('emits kind=unknown success=false for an URI that matches nothing', async () => {
    ...
    await handleMcpRequest(
      mcpRequest('resources/read', { uri: 'Nordklart://nonexistent/whatever' })
    )

    const event = await eventPromise
    expect(event.uri).toBe('Nordklart://nonexistent/whatever')
  ...
  it('emits kind=unknown for a skill URI with an unknown slug', async () => {
    ...
    await handleMcpRequest(
      mcpRequest('resources/read', { uri: 'Nordklart://skill/does-not-exist' })
    )
