...
describe('VAT review widget', () => {
  ...
  describe('nordklart_vat_review_widget tool', () => {
    it('is registered with _meta.ui pointing to the vat-review widget', () => {
      const tool = tools.find((t) => t.name === 'nordklart_vat_review_widget')
      expect(tool).toBeDefined()
      expect(tool?._meta).toEqual({ ui: { resourceUri: 'ui://vat-review/app.html' } })
      expect(tool?.annotations.readOnlyHint).toBe(true)
    })

    it('declares the same required inputs as nordklart_get_vat_report', () => {
      const widgetTool = tools.find((t) => t.name === 'nordklart_vat_review_widget')
      const reportTool = tools.find((t) => t.name === 'nordklart_get_vat_report')
  ...
  describe('protocol: tools/list', () => {
    it('includes nordklart_vat_review_widget with _meta when the API key has reports:read scope', async () => {
      const res = await handleMcpRequest(mcpRequest('tools/list'))
      const result = await parseResult(res)

      const widgetTool = result.tools.find(
        (t: { name: string }) => t.name === 'nordklart_vat_review_widget'
      )
  ...
  describe('nordklart_get_vat_report — render_ui merge', () => {
    it('declares render_ui and points at the vat-review widget', () => {
      const tool = tools.find((t) => t.name === 'nordklart_get_vat_report')!
