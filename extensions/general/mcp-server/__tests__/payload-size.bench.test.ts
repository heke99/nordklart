...
describe('tools/list payload size guard', () => {
  it('keeps the projected tools/list payload under the context-budget ceiling', () => {
    ...
    //     ~75 to 83 tools (added nordklart_create_supplier, nordklart_list_pending_operations,
    //     nordklart_approve_pending_operation, nordklart_reject_pending_operation,
    //     nordklart_set_inbox_extracted_data from main + nordklart_get_agent_briefing,
    //     _remember_fact, _forget_fact, _feedback from the agent branch).
    //   * 30K → 31K when nordklart_match_batch_allocate and
    //     nordklart_bulk_book_transactions landed (PRs #603/#606/#608/#610). Each
    ...
    //   * 31K → 31.5K when nordklart_link_transaction_to_journal_entry landed (PR
    ...
    //   * 31.5K → 32K when nordklart_find_voucher_candidates_for_supplier_invoice +
    //     nordklart_link_supplier_invoice_to_voucher landed — the supplier-side
    ...
    // Long-term answer to growth is leaning harder on nordklart_search_tools — if this
