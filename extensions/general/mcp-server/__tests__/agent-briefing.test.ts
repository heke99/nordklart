/**
 * Tests for nordklart_get_agent_briefing — session-bootstrap context for the
...
describe('nordklart_get_agent_briefing tool', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('is registered with readOnly + idempotent annotations', () => {
    const tool = tools.find((t) => t.name === 'nordklart_get_agent_briefing')
