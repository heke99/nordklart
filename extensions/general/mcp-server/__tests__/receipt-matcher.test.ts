...
describe('MCP Receipt Matcher', () => {
  ...
  describe('tools/list', () => {
    it('includes _meta.ui for nordklart_receipt_matcher', async () => {
      const res = await handleMcpRequest(mcpRequest('tools/list'))
      const result = await parseResult(res)

      const receiptTool = result.tools.find((t: { name: string }) => t.name === 'nordklart_receipt_matcher')
    ...
    it('does not include _meta for tools without it', async () => {
      ...
      const categorizeTool = result.tools.find(
        (t: { name: string }) => t.name === 'nordklart_categorize_transaction'
      )
  ...
  describe('resources/list', () => {
    it('includes the receipt-matcher widget alongside data resources', async () => {
      ...
      expect(uris).toContain('Nordklart://company/current')
      expect(uris).toContain('Nordklart://capabilities')
    })
  })
  ...
  // ── nordklart_receipt_matcher tool ──

  describe('nordklart_receipt_matcher', () => {
    it('returns uncategorized transactions with categories and vat_treatments', async () => {
      ...
      const res = await handleMcpRequest(
        mcpRequest('tools/call', { name: 'nordklart_receipt_matcher', arguments: {} })
      )
      const result = await parseResult(res)
  ...
  // ── nordklart_categorize_transaction still works after refactor ──

  describe('nordklart_categorize_transaction (staging)', () => {
