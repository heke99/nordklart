/**
 * Staging-time gate tests for nordklart_create_voucher.
 *
...
const createVoucher = tools.find((t) => t.name === 'nordklart_create_voucher')!
const correctEntry = tools.find((t) => t.name === 'nordklart_correct_entry')!
const reverseEntry = tools.find((t) => t.name === 'nordklart_reverse_journal_entry')!
...
describe('nordklart_create_voucher — staging gates', () => {
  it('is registered and mapped to bookkeeping:write scope', async () => {
    ...
    expect(TOOL_SCOPE_MAP.nordklart_create_voucher).toBe('bookkeeping:write')
  })
...
describe('nordklart_correct_entry — registration', () => {
  it('is registered with bookkeeping:write scope and is not read-only', async () => {
    ...
    expect(TOOL_SCOPE_MAP.nordklart_correct_entry).toBe('bookkeeping:write')
  })
...
describe('nordklart_reverse_journal_entry — staging gates', () => {
  it('is registered with bookkeeping:write scope and is not read-only', async () => {
    const { TOOL_SCOPE_MAP } = await import('@/lib/auth/api-keys')
    expect(reverseEntry).toBeDefined()
    expect(reverseEntry.annotations.readOnlyHint).toBe(false)
    expect(TOOL_SCOPE_MAP.nordklart_reverse_journal_entry).toBe('bookkeeping:write')
  })
...
describe('entry_id resolution — voucher refs and hallucinated UUIDs', () => {
  ...
  // nordklart_correct_entry. The same resolution path is wired into
  // nordklart_reverse_journal_entry, so one tool is enough to lock the behaviour.
...
describe('agent memory tools — agent:write scope gate', () => {
  const rememberFact = tools.find((t) => t.name === 'nordklart_remember_fact')
  const forgetFact = tools.find((t) => t.name === 'nordklart_forget_fact')
  ...
  it('registers the memory write tools mapped to agent:write', async () => {
    ...
    expect(TOOL_SCOPE_MAP.nordklart_remember_fact).toBe('agent:write')
    expect(TOOL_SCOPE_MAP.nordklart_forget_fact).toBe('agent:write')
  })
  ...
  it('denies a key without agent:write and allows one with it', async () => {
    ...
    const required = TOOL_SCOPE_MAP.nordklart_remember_fact
