...
describe('withNext', () => {
  ...
  it('attaches next hint when provided', () => {
    const result = withNext(
      { id: 'x' },
      { description: 'Send the invoice', tool: 'nordklart_send_invoice' }
    )
    expect(result).toEqual({
      data: { id: 'x' },
      next: { description: 'Send the invoice', tool: 'nordklart_send_invoice' },
    })
  })
})
...
describe('toToolError', () => {
  it('produces structured error from arbitrary throw', () => {
    ...
    expect(result.error.remediation?.tool).toBe('nordklart_lock_period')
  })
