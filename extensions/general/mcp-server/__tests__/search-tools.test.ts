...
const searchTool = tools.find((t) => t.name === 'nordklart_search_tools')!
...
describe('nordklart_search_tools', () => {
  ...
  it('ranks by relevance — the most on-point tool comes first', async () => {
    ...
    expect(vat.tools[0].name).toBe('nordklart_get_vat_report')
    ...
    expect(paid.tools[0].name).toBe('nordklart_mark_invoice_as_paid')
  })
  ...
  it('filters out tools the caller cannot invoke based on scopes', async () => {
    ...
    expect(names).not.toContain('nordklart_create_invoice')
    expect(names).not.toContain('nordklart_send_invoice')
    // But should see reports:read tools.
    expect(names).toContain('nordklart_get_trial_balance')
    // And unscoped tools (like search itself) are always available.
    expect(names).toContain('nordklart_search_tools')
  })
  ...
  it('fail-closed: hides scoped tools when __keyScopes is not injected', async () => {
    ...
    expect(names).toContain('nordklart_search_tools')
    expect(names).toContain('nordklart_list_skills')
    expect(names).toContain('nordklart_load_skill')

    // No scoped tool should leak — pick representatives from each scope domain.
    expect(names).not.toContain('nordklart_create_invoice')         // invoices:write
    expect(names).not.toContain('nordklart_get_trial_balance')      // reports:read
    expect(names).not.toContain('nordklart_list_uncategorized_transactions') // transactions:read
    expect(names).not.toContain('nordklart_create_salary_run')      // payroll:write
    expect(names).not.toContain('nordklart_close_period')           // bookkeeping:write
  ...
  it('fail-closed: explicitly empty __keyScopes also hides scoped tools', async () => {
    ...
    const names = result.tools.map((t) => t.name)
    expect(names).not.toContain('nordklart_create_invoice')
    expect(names).not.toContain('nordklart_get_trial_balance')
    for (const t of result.tools) {
      expect(t.scope).toBeNull()
    }
  })
