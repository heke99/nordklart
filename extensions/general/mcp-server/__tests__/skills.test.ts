...
/** Build a supabase mock that satisfies the queries nordklart_list_skills issues:
...
describe('Skills registry', () => {
  ...
  it('skillUri uses the Nordklart://skill/ prefix; atom slugs are URL-encoded', () => {
    expect(skillUri('foo')).toBe('Nordklart://skill/foo')
    expect(skillUri('vertical/konsult-it')).toBe('Nordklart://skill/vertical%2Fkonsult-it')
    expect(SKILL_URI_PREFIX).toBe('Nordklart://skill/')
  })
})

describe('nordklart_list_skills tool', () => {
  beforeEach(() => {
    __resetAtomCache()
  })

  it('is registered with correct annotations and no scope requirement', () => {
    const tool = tools.find((t) => t.name === 'nordklart_list_skills')
...
describe('nordklart_load_skill tool', () => {
  beforeEach(() => {
    __resetAtomCache()
  })

  it('is registered', () => {
    const tool = tools.find((t) => t.name === 'nordklart_load_skill')
    expect(tool).toBeDefined()
  })
...
describe('Skills via MCP protocol', () => {
  ...
  it('resources/list includes one entry per skill at Nordklart://skill/<slug>', async () => {
  ...
  it('resources/read returns the Markdown body for a skill URI', async () => {
    ...
    expect(result.contents[0].uri).toBe('Nordklart://skill/quarterly-vat-review')
