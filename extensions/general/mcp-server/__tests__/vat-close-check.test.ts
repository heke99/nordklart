/**
 * Unit tests for nordklart_vat_close_check.
 *
...
describe('nordklart_vat_close_check', () => {
  it('is registered in the tools array', () => {
    const tool = tools.find((t) => t.name === 'nordklart_vat_close_check')
  ...
  it('is mapped to reports:read scope', () => {
    expect(TOOL_SCOPE_MAP.nordklart_vat_close_check).toBe('reports:read')
  })
})
