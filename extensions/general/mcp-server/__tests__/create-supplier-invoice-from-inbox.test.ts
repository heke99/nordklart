/**
 * Unit tests for nordklart_create_supplier_invoice_from_inbox.
 *
...
describe('nordklart_create_supplier_invoice_from_inbox — registration', () => {
  it('is registered with idempotent + non-read-only annotations', () => {
    const tool = tools.find((t) => t.name === 'nordklart_create_supplier_invoice_from_inbox')
  ...
  it('is mapped to suppliers:write scope', () => {
    expect(TOOL_SCOPE_MAP.nordklart_create_supplier_invoice_from_inbox).toBe('suppliers:write')
  })
...
describe('nordklart_create_supplier_invoice_from_inbox — execute', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
