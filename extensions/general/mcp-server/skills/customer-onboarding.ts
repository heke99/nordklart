...
const body = `# Customer Onboarding — Nordklart
...
- Before \`nordklart_create_invoice\` whenever the customer isn't found in \`nordklart_list_customers\`
...
- **B2G (Swedish municipality/agency)**: still \`swedish_business\`, but the invoice must be Peppol-formatted (legal requirement since 2019-04-01). Nordklart handles Peppol when the customer record has a Peppol endpoint configured.
...
\`nordklart_list_customers\` returns all customers. Search by name client-side.
...
Nordklart's \`nordklart_create_customer\` runs VIES automatically when \`vat_number\`
...
If VIES is temporarily down (intermittent), Nordklart marks the customer as
...
appears in \`nordklart_list_customers\` and can be invoiced via \`nordklart_create_invoice\`.

### Step 5 — Sanity-check before first invoice

\`nordklart_list_customers\` — confirm the customer appears, customer_type is right.
Read \`vat_number_validated\` (for EU); if false, double-check the VAT number.

When you eventually call \`nordklart_create_invoice\`, Nordklart uses the
...
- *"VAT rate 25% is not allowed for customer type eu_business"*: the customer's VAT number is validated → reverse charge required. Use 0 % on line items, and Nordklart will add the "Omvänd betalningsskyldighet" notation automatically.
- *"VAT rate 0% is not allowed for customer type swedish_business"*: domestic customers always get 25/12/6 %. Pick the right rate.
- *"Customer not found"* on \`nordklart_create_invoice\`: the customer wasn't created (or its pending_operation wasn't approved). Run \`nordklart_list_customers\` to confirm.
...
- \`nordklart_list_customers\` — check for existing customers
- \`nordklart_create_customer\` — stage the new customer
- \`nordklart_create_invoice\` — uses the customer_type for VAT-rate validation
