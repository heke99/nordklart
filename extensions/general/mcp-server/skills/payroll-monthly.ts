...
const body = `# Monthly Payroll — Nordklart
...
\`nordklart_list_employees\` returns all active employees. For each, the system needs:
...
\`nordklart_create_salary_run({ period_year, period_month, payment_date })\`
...
\`nordklart_calculate_salary_run({ salary_run_id })\`. Computes per employee:
...
\`nordklart_get_salary_run({ salary_run_id })\` — full breakdown including \`calculation_breakdown\` showing step-by-step formulas. The user reviews per-employee in web UI.

\`nordklart_get_salary_journal({ year })\` — annual rollup for sanity check.
...
When a run mixes full-rate and reduced-rate employees, the 7510/2730 lines are summed across all employees — the *total* avgift line equals \`Σ(per-employee avgift_base × per-employee rate)\`, **not** \`Σ bruttolön × 31.42 %\`. \`nordklart_calculate_salary_run\` already does this aggregation.

### Step 7 — Generate AGI

\`nordklart_generate_agi({ salary_run_id })\`. Run must be in \`review\`/\`approved\`/\`paid\`/\`booked\` status (past draft).
...
- **Run already exists**: idempotency on (company, year, month). Find the existing run with \`nordklart_get_salary_run\`.
...
- \`nordklart_list_employees\` — verify setup
- \`nordklart_create_salary_run\` — stage new monthly run
- \`nordklart_calculate_salary_run\` — compute tax + avgifter + accrual
- \`nordklart_get_salary_run\` — review breakdown
- \`nordklart_get_salary_journal\` — annual rollup
- \`nordklart_generate_agi\` — produce AGI XML for filing
`
