...
const body = `# Quarterly VAT Review — Nordklart
...
\`nordklart_get_vat_report({ period_type: 'quarterly', year: YYYY, period: 1|2|3|4 })\`

Returns all rutor (boxes) plus a summary string.

### Step 3 — Visual review

\`nordklart_vat_review_widget(...)\` opens a tabular UI. The user reviews each ruta inline, copies the summary, and confirms before filing.

### Step 4 — Drill into anomalies

If any ruta looks wrong, call \`nordklart_get_general_ledger\` filtered to the relevant 26xx account:
...
- \`nordklart_get_vat_report\` — generate momsdeklaration data
- \`nordklart_vat_review_widget\` — interactive review widget
- \`nordklart_get_general_ledger\` — drill into 26xx accounts
- \`nordklart_list_uncategorized_transactions\` — verify nothing missing
- \`nordklart_get_reconciliation_status\` — bank vs ledger sanity check
`
