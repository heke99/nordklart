...
const body = `# Kreditfaktura (Credit Note) Process — Nordklart
...
| Situation | Tool | Why |
|---|---|---|
| Invoice was sent to customer; needs partial/full reversal | \`nordklart_credit_invoice\` | Creates a KR-prefixed mirror invoice + reverses the original JE. Customer gets a kreditfaktura PDF. |
| Internal verifikation that has wrong account/amount but no customer-facing invoice involved | \`nordklart_correct_entry\` | Storno + new posted entry. No invoice document changes. Stays inside BFL 5 kap. 5 §. |
| Internal verifikation that should never have existed (duplicate, ghost) | \`nordklart_reverse_journal_entry\` | Pure storno — original stays visible, no replacement posted. |
...
\`nordklart_list_invoices(status='sent')\` or \`status='overdue'\` to find the invoice.
...
- A proforma (use \`nordklart_convert_invoice\` first if needed, but proformas don't post to VAT/AR — usually you just cancel them)

### Step 2 — Stage the kreditfaktura

\`nordklart_credit_invoice({ invoice_id, reason?: 'Felaktigt belopp' })\`.
...
- Issue an outbound bank payment for the refund amount. When the bank shows
  the outgoing transaction, categorize it via \`nordklart_categorize_transaction\`
...
\`nordklart_send_invoice({ invoice_id: <kreditfaktura_id> })\`.
...
reference the document being credited — Nordklart does this automatically.

### Step 5 — Verify

- \`nordklart_get_ar_ledger\` — customer balance should reflect the credit.
- \`nordklart_get_vat_report(period_type='monthly', year, period)\` — the credit reduces ruta 05 + ruta 10/11/12 (output VAT) in the current period.
- \`nordklart_get_general_ledger(account_number='1510')\` (or 3xxx revenue accounts) — confirm the storno hit.

## Partial credits

If only part of the invoice should be credited (e.g. one of three items was
defective), the current \`nordklart_credit_invoice\` always credits the FULL
original. For partial credits, the current workaround:

1. Credit the full invoice with \`nordklart_credit_invoice\`.
2. Issue a new invoice for the items that ARE still owed via \`nordklart_create_invoice\`.
...
- \`nordklart_list_invoices\` — find the original
- \`nordklart_credit_invoice\` — the main tool, stages the kreditfaktura
- \`nordklart_send_invoice\` — deliver the credit note to the customer
- \`nordklart_categorize_transaction\` / \`nordklart_match_transaction_to_invoice\` — book the refund payment (if any)
- \`nordklart_get_ar_ledger\` / \`nordklart_get_vat_report\` — verification
`
