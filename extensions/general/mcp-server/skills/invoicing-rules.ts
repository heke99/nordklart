...
const body = `# Invoicing Rules — Nordklart
...
The \`nordklart_create_invoice\` tool handles all of these automatically — but always provide \`our_reference\`/\`your_reference\` if known.

## Workflow

### Step 1 — Customer ready

Customers with their full data already in the system: \`nordklart_list_customers\`. Find the one. Note the \`customer_id\`.

If the customer doesn't exist:

\`nordklart_create_customer\` with at minimum \`{ name, customer_type }\`. \`customer_type\` must be one of:
...
Use \`getAvailableVatRates(customerType, vatNumberValidated)\` semantics — Nordklart handles this. Per-line override possible via \`vat_rate\` on each item.

### Step 3 — Create the invoice

\`nordklart_create_invoice({ customer_id, items: [{ description, quantity, unit, unit_price, vat_rate? }], invoice_date?, due_date?, currency? })\`
...
\`nordklart_send_invoice(invoice_id)\` — emails the PDF to the customer. Requires email service configured (Resend) and customer email on file.

If the user delivered the invoice manually (printed, e-faktura via Peppol, etc.), use \`nordklart_mark_invoice_as_sent\` instead — same booking effect, no email.

### Step 5 — Record payment

When money arrives in 1930:

- **Match to bank transaction** (preferred): \`nordklart_match_transaction_to_invoice({ transaction_id, invoice_id })\` — links the payment, marks invoice paid (or partially_paid), books JE.
- **Manual mark**: \`nordklart_mark_invoice_as_paid({ invoice_id, payment_date })\` — when payment arrived but isn't in the bank feed yet.

### Step 6 — Reverse if needed

If the invoice was wrong: \`nordklart_credit_invoice({ invoice_id, reason })\` creates a \`KR-\` mirror invoice with negated amounts and reverses the original JE. Original status → \`credited\`. **Never edit a sent invoice** — kreditfaktura is the only legal path.

The kreditfaktura **itself** consumes a sequential number from the same (or a dedicated KR-) fakturaserie per BFL 5 kap. 6–7 § / ML 17 kap. 22–23 §. The \`KR-\` prefix is a display convention; the underlying löpnummer must be unbroken just like the regular invoice series. \`nordklart_credit_invoice\` allocates this atomically at approval — agents shouldn't try to set or skip the number manually.
...
This data goes on the invoice; Nordklart's invoice template renders it automatically when set on the customer.

## Peppol / e-invoicing (B2G)

Swedish authorities require e-invoices via Peppol BIS Billing 3.0 (Lag 2018:1277). For private B2B, the buyer's preference governs but Peppol is preferred. Nordklart renders an EN 16931-compliant XML on demand.

## Critical rules

- **Invoice numbers are sequential and gap-free.** Allocated atomically at approval. If you change your mind, use \`nordklart_credit_invoice\`, never delete or skip a number — Skatteverket will audit.
- **F-skatt notation is mandatory** for B2B services. Nordklart adds it automatically when company settings have F-skatt = true.
...
- **Sent before approval**: not possible — \`nordklart_send_invoice\` stages too. The user must approve.
- **Edit instead of credit**: blocked by DB triggers. Use \`nordklart_credit_invoice\`.

## Tools

- \`nordklart_list_customers\` / \`nordklart_create_customer\` — customer setup
- \`nordklart_create_invoice\` — stage new invoice
- \`nordklart_send_invoice\` — email PDF
- \`nordklart_mark_invoice_as_sent\` — manual delivery
- \`nordklart_mark_invoice_as_paid\` — manual payment
- \`nordklart_match_transaction_to_invoice\` — link bank payment
- \`nordklart_credit_invoice\` — kreditfaktura (legal undo)
- \`nordklart_convert_invoice\` — proforma → real invoice
- \`nordklart_list_invoices\` — find existing invoices
`
