...
const body = `# Year-End Close (Bokslut) — Nordklart
...
If the company has open foreign-currency receivables/payables (1510/2440 in EUR/USD/etc.), revalue to closing-date FX rate via \`nordklart_run_currency_revaluation({ fiscal_period_id, closing_date })\`. Posts to **3960** (kursvinster) and **7960** (kursförluster). One revaluation per period.

### Step 3 — Lock the period

\`nordklart_lock_period(fiscal_period_id)\`. Required before year-end. Refuses if business transactions are unbooked.

### Step 4 — Run year-end

\`nordklart_run_year_end(fiscal_period_id)\` — stages a high-risk operation. After approval:
...
\`nordklart_set_opening_balances({ closed_period_id, next_period_id })\`. Copies class 1–2 closing balances into the next period as opening balances. Stage → approve.

### Step 6 — Close (final, irreversible)

\`nordklart_close_period(fiscal_period_id)\`. Once approved, the period is sealed forever. **No more entries possible — not even via storno.**
...
These compute with \`nordklart_get_kpi_report\` for inputs but the actual tax JE is web-UI today.

## Critical rules

- **Year-end is forever.** Once \`nordklart_close_period\` succeeds, there is no rollback. \`nordklart_unlock_period\` cannot unlock a closed period — only one that is locked but not closed.
...
- \`nordklart_lock_period\` — pre-flight before year-end
- \`nordklart_run_year_end\` — zero result accounts
- \`nordklart_set_opening_balances\` — seed next period
- \`nordklart_run_currency_revaluation\` — FX revaluation
- \`nordklart_close_period\` — final, irreversible
- \`nordklart_get_balance_sheet\` — verify post-year-end balances
- \`nordklart_get_income_statement\` — verify result before year-end JE
- \`nordklart_get_trial_balance\` — sanity check before each step
`
