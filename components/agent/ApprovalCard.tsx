...
// endpoints — same surface the Nordklart "Förslag" page uses, so there is
...
// nordklart_get_recent_rejections.
...
export default function ApprovalCard({
  ...
  if (state === 'committed') {
    ...
    // Open the Nordklart web app to approve or reject it.") was written for
...
function PreviewBlock({ toolName, preview }: PreviewBlockProps) {
  ...
  if (toolName === 'nordklart_categorize_transaction') {
    return <CategorizeTransactionPreview preview={p} />
  }
  if (toolName === 'nordklart_create_invoice') {
    return <CreateInvoicePreview preview={p} />
  }
  if (toolName === 'nordklart_create_voucher' || toolName === 'nordklart_correct_entry') {
