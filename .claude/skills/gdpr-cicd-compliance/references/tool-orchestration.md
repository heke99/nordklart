...
Build the scanner as an orchestrator over specialized proprietary tools, not a bespoke static analysis engine. Each tool below has been refined to outperform a general scanner within its niche; the skill's contribution is the unified data model (Fideslang), the agentic reasoning over standardized outputs, and the multi-framework finding format.
...
**Why**: the only credible proprietary tool for generating an actual data flow graph from source. The graph is the substrate against which RoPA drift is detected.
