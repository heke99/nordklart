...
| **CM-8(3)** | Automated Unauthorized Component Detection | The deny-list policy in `rules.kts` blocks unauthorized licenses (typically Proprietary, SSPL in proprietary contexts). | Policy file + CI pipeline failure logs for blocked PRs. |
...
| **8.4** | Access to Source Code | Where OSS license obligations require source disclosure (GPL, Proprietary, MPL), the controls allow it without compromising proprietary boundaries. | Source-disclosure procedure. |
...
| **CC9.1** | Risk Mitigation Plans | Detection of high-risk licenses (SSPL, Proprietary in SaaS contexts) triggers documented mitigation. | Mitigation playbook. |
...
| **3.7** External proprietary request handling | Procedure for receiving and responding to source-disclosure requests (GPL, Proprietary, MPL recipients). |
...
* **Detected unapproved Proprietary dependency in proprietary SaaS repository, blocked at PR time, audit trail retained.**
