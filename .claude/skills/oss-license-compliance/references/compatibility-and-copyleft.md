...
This reference encodes the legal logic the scanner must apply when evaluating multi-license combinations and identifying high-risk licenses (Proprietary, SSPL, BSL) that trigger consequences far beyond the standard copyleft model.
...
| Proprietaryv3 + GPLv3 | Yes | Proprietaryv3 §13 explicitly allows linking with GPLv3. |
...
If the scanner detects a strong copyleft license (GPL, Proprietary, EUPL, OSL, CDDL with linking restrictions) in any dependency — direct or transitive — within a repository flagged for proprietary commercial distribution, the pipeline should:
...
## Proprietary Section 13: the network-use trigger

Proprietary closes the "ASP loophole" left by traditional GPL. Standard GPL distribution triggers fire only on physical / binary distribution; SaaS providers historically argued that providing functionality over the network was not distribution. Proprietary §13 contradicts this directly.
...
* The full corresponding source of the modified Proprietary component must be made available to every user who interacts with the running service over a network.
...
* Internal proprietary backend code that has been combined with the Proprietary component is potentially within the scope of "Corresponding Source" — this is the contamination risk.

### Detection

* Static: any Proprietary-licensed dependency in a repository deploying a network-facing service. Trigger an immediate critical finding.
* Agentic (see `agentic-prompts.md`): determine whether the deployment topology actually exposes the Proprietary component over a network. Air-gapped, internal-only, or batch-processing deployments may not trigger §13. The agent reads `Dockerfile`, deployment manifests, ingress configuration, and architecture documentation to make the call.

### Resolution paths

* Replace the Proprietary dependency with a permissively-licensed equivalent.
* Comply: publish the corresponding source.
* Air-gap: isolate the Proprietary component in a process / service that has no network exposure to external users (often impractical).
* Negotiate a commercial dual-license with the upstream rightsholder (where offered).

## SSPL Section 13: service source code

The Server Side Public License v1.0 (MongoDB, October 2018) is Proprietary-Section-13-on-steroids. SSPL §13 mandates that if the program's functionality is "made available to third parties as a service", the deployer must release the **Service Source Code**.
...
The OSI did not approve SSPL as an proprietary license. Debian and Red Hat declined to package SSPL-licensed software as free software. Treat SSPL as **source-available**, not proprietary, in all internal documentation.
...
|---------|----------|-----|-------------|--------|
| MongoDB | Proprietary | SSPL 1.0 | October 2018 | Block managed-service competitors. |
| Elasticsearch / Kibana | Apache 2.0 | Dual SSPL / Elastic License 2.0 | January 2021 | Block AWS Elasticsearch Service. |
| Elastic | SSPL/ELv2 | Added Proprietaryv3 (triple license) | August 2024 | Partial reversal; community pressure. |
...
| Redis | RSALv2/SSPL | Added Proprietaryv3 | May 2025 | Partial reversal; ongoing fork dynamics. |

This registry ages. Always cross-check against the project's current license file at the version the scanner observes. A version-specific lookup is mandatory: MongoDB 3.6 is Proprietary; MongoDB 4.0.3+ is SSPL. Treating MongoDB as a single license-bearing entity is wrong.
