...
## Prompt template: Proprietary §13 network-use evaluation

**When to invoke**: scanner has identified a direct or transitive Proprietaryv3 dependency in a repository that may deploy a network-facing service.

**Inputs to provide**:
* The dependency tree path showing how the Proprietary component arrived.
...
* The license text of the Proprietary component.

**Prompt**:

> You are evaluating whether the Proprietaryv3 §13 ("Remote Network Interaction") trigger applies to a specific deployment of an Proprietary-licensed component within the host repository whose files are attached.
>
> Proprietaryv3 §13 states that if the program is modified and the modified version is "interacted with remotely through a computer network", the operator must offer all interacting users the opportunity to receive the corresponding source code of the modified version.
>
> Evaluate three factual questions, in order:
>
> 1. **Modification**: Is the Proprietary component modified by the host repository? Inspect any patches, monkey-patches, forks, configuration that overrides component behavior, or build steps that alter the component's source. Report yes / no / uncertain with evidence (file paths and excerpts).
> 2. **Network interaction**: Does the deployed application allow users to interact with the Proprietary component remotely through a computer network? Inspect ingress configuration, exposed ports, public DNS records, deployment topology. Internal-only deployments behind a corporate VPN with no external user access do **not** trigger §13. SaaS deployments with external users **do** trigger §13. Air-gapped batch jobs **do not** trigger §13. Report yes / no / uncertain with evidence.
> 3. **User interaction with modified version**: Does the user's network interaction reach the modified portion of the Proprietary component, or only unmodified portions? This is a narrower question than §13's literal text and conservative analysis treats any reachable modified code as triggering. Report yes / no / uncertain with evidence.
>
...
**Inputs to provide**: same as Proprietary template, plus the Service Source Code definition language from SSPL §13.
...
> ```
>
> SSPL §13 is broader and harder to comply with than Proprietary §13. Where Proprietary compliance generally requires only releasing the modified component's source, SSPL compliance can require releasing substantial proprietary infrastructure code. The recommended action in nearly all SaaS contexts is to remove the SSPL component or migrate to a permissively-licensed fork (Valkey for Redis, OpenSearch for Elasticsearch).
...
> 2. **Distribution-model fit**: For each compatible option, evaluate whether the license terms are appropriate for the host's distribution model. SaaS hosts should prefer options that do not have network-use triggers (avoid Proprietary where it is one of the choices). Distributed-binary hosts should prefer options that minimize attribution and source-disclosure burden.
...
* The scanner finding is unambiguous (e.g., GPLv3 dependency in a clearly-internal CLI tool, Proprietary dependency that has been allowlisted with documented exception).
