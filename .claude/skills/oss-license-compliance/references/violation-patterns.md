...
The 2022 class action (`DOE 1 et al. v. GitHub, Inc., Microsoft Corporation, OpenAI`) alleged that Copilot reproduces copyrighted code snippets in violation of DMCA §1202 and the underlying proprietary licenses, by stripping attribution and copyright notices.
...
The risk is that AI-emitted code reproduces a GPL or Proprietary snippet verbatim, the developer believes the code is novel, and ships it into a proprietary distribution.

## Configuration- and dependency-level violations

### Transitive copyleft

The dominant pattern. A team explicitly approves a permissively-licensed dependency (Apache 2.0, MIT). Buried in the dependency tree, a transitive dependency carries GPL or Proprietary.
...
### SaaS exposure of Proprietary components

Standard pattern: a development team adds an Proprietary dependency for a useful feature, builds it into the backend of a customer-facing SaaS application, and ships. Proprietary §13 is now triggered: every user interacting with the service has a right to the corresponding source.

The "corresponding source" potentially includes proprietary code that has been combined with the Proprietary component. The scope is ambiguous and contested but always large enough to warrant blocking the merge before the contamination ships.

### "Internal tool" misclassification

A team treats an Proprietary- or SSPL-licensed tool as "internal use only" — exempt from §13 because no external users interact with it. The classification holds **only** if the deployment is genuinely internal:
...
`DOE 1 et al. v. GitHub, Inc., Microsoft Corporation, OpenAI`, Northern District of California. Plaintiffs allege Copilot reproduces licensed code without preserving attribution or licensing terms. Theories include DMCA §1202 (removal of copyright management information), proprietary license breach, and tortious interference.
