...
description: "Triage Nordklart customer support emails and turn them into GitHub issues in the nordklart/nordklart repo. Use this skill whenever the user invokes /supportmail-to-ticket (with or without a number argument), or asks to 'triage support mail', 'turn support emails into tickets', 'process Nordklart support', 'check the support inbox and file issues', or any similar phrasing involving the Nordklart support mailbox. Also trigger this skill if the user mentions [Nordklart support] emails and wants them converted into actionable work — even if they don't use the exact slash command."
---

# supportmail-to-ticket

Triage `[Nordklart support]` emails from Gmail, cross-reference them against the local nordklart codebase, and draft GitHub issues for the `nordklart/nordklart` repo — with inline user approval before anything gets created.
...
- `N` = number of most recent `[Nordklart support]` threads to triage. Optional. Default: `3`.
...
- **Filesystem MCP** pointing at `C:\Users\emilm\projects\nordklart` — the skill is explicitly designed around local code search. If the filesystem tool is not available or the path doesn't resolve, stop and say: *"I need the Filesystem MCP configured with access to `C:\Users\emilm\projects\nordklart`. Please set it up in Claude Desktop's MCP config and retry."* Do not fall back to remote code search — the user has asked for local-only.
...
- `query`: `subject:"[Nordklart support]"`
...
For each email, analyze the complaint and search the local codebase at `C:\Users\emilm\projects\nordklart`.
...
```bash
gh issue list --repo nordklart/nordklart --state open --limit 100 --json number,title,body,url
```
...
- You can check available labels once at the start of phase 5 with: `gh label list --repo nordklart/nordklart --limit 100 --json name` — useful if multiple label errors happen in a row.

**Project board**: Issues are created in the `nordklart/nordklart` repo. The Nordklart project board (`erp-mafia/projects/...`) aggregates issues but adding to a project via `gh` requires `gh project item-add` with the project number and GraphQL scopes that may not be in the current auth token. After creating issues, output the project URL once and remind the user they may want to drag the new issues onto the board. Example wording: *"Issues created. If you want them on the Nordklart project board, you'll need to add them manually at https://github.com/orgs/erp-mafia/projects — or run `gh project item-add` if you have project scopes on your token."*
...
```
Created 2 issues:
- #47 Fix SIE import failure for first fiscal year — https://github.com/nordklart/nordklart/issues/47
- #48 Investigate bank import for banks without BankID — https://github.com/nordklart/nordklart/issues/48
