...
- **Oxalis-NG receive on day one** is proprietary-friendly, scales linearly without per-doc fees, and gives genuine "native Peppol receive" differentiation that incumbents (white-labelling Crediflow/InExchange) cannot match without rebuilding their stack.
...
**Total go-live cost** for a credible, differentiated, proprietary native-Peppol Swedish bookkeeping product: **approximately €10–15k plus 3 months engineering**.
...
- **The proprietary angle is force multiplier**: publishing the UBL generator on GitHub under BSD/MIT earns trust with bookkeeping/Linux-friendly customers without surrendering the moat, which lives in the bookkeeping engine and UX, not the XML serialiser.
